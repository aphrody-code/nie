#!/usr/bin/env python3
"""Validation byte-exact : SECONDE instanciation du conteneur map intrusive (`FUN_14050b0b0`).

`FUN_14050b0b0` est le MÊME conteneur dual-mode que `FUN_1402e2a10`(+`FUN_1402b4160`) — header
identique (+8 base, +0x10 tête, +0x14 count, +0x1c nodes_off, +0x20 sorted_off, +0x24 cap), nœuds
6 o (next u16 @+2), recherche binaire trié — à UN détail près : **entrées de 0xc o** (clé i32 @+0)
au lieu de 0x10. Il renvoie directement le NŒUD (entrée résolue) dans les deux modes.

But : prouver que le port Rust généralisé (`IntrusiveMap{Linear,Sorted}` paramétré par `entry_stride`)
est byte-exact contre une DEUXIÈME fonction réelle → unification + double-validation anti-faux-FAIT.
Les miroirs (linéaire indexé par nœud ; trié sur clés triées) sont stride-agnostiques. Sort 0 si
byte-exact. Fuzz seedé des deux modes.
"""
import struct
import sys
...
CAP = 0x100
STRIDE = 0xC  # ← la seule différence avec la famille 0x10 déjà portée
...
def emu_linear(e, entries, nexts, head, key):
    ...
    for i, k in enumerate(entries):
        struct.pack_into("<i", data, 8 + i * STRIDE, k)
    for i, nx in enumerate(nexts):
        struct.pack_into("<H", data, NOFF + i * 6 + 2, nx)
    out = e.call(0x14050B0B0, rcx=HDR, rdx=key & 0xFFFFFFFF, mem={HDR: hdr, BASE: bytes(data)})
    rax = out["rax"]
...
def emu_sorted(e, keys, perm, target):
    ...
    for i, p in enumerate(perm):
        struct.pack_into("<I", data, 8 + p * STRIDE, keys[i] & 0xFFFFFFFF)
        struct.pack_into("<H", data, SOFF + i * 2, p)
    out = e.call(0x14050B0B0, rcx=HDR, rdx=target & 0xFFFFFFFF, mem={HDR: hdr, BASE: bytes(data)})
    rax = out["rax"]
...
print(f"intrusive_map (stride 0xc, FUN_14050b0b0): {total} cas ; {'BYTE-EXACT' if not fails else f'{fails} DIVERGENCES'}")
