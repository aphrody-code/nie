#!/usr/bin/env python3
"""Validation byte-exact : QUATRIÈME instanciation du conteneur map intrusive (`FUN_1404523c0`).

`FUN_1404523c0` est le MÊME conteneur dual-mode que `FUN_1402e2a10`/`FUN_14050b0b0` — header
identique (+8 base, +0x10 tête, +0x14 count, +0x1c nodes_off, +0x20 sorted_off, +0x24 cap), nœuds
6 o (next u16 @+2), recherche binaire trié — à DEUX détails près : **entrées de 0x28 o** ET **clé
...
ET `key_base`) est byte-exact contre une QUATRIÈME fonction réelle → le conteneur est un *template*
...
HDR = SCRATCH + 0x100
BASE = SCRATCH + 0x4000
NOFF = 0x600  # déplacé : stride 0x28 × ~20 entrées dépasse 0x200 (collision entrées/nœuds)
SOFF = 0x800  # mode trié (après les nœuds)
CAP = 0x100
STRIDE = 0x28  # 4e instanciation
...
def emu_linear(e, entries, nexts, head, key):
    ...
    out = e.call(0x1404523C0, rcx=HDR, rdx=key & 0xFFFFFFFF, mem={HDR: hdr, BASE: bytes(data)})
    rax = out["rax"]
...
def emu_sorted(e, keys, perm, target):
    ...
    out = e.call(0x1404523C0, rcx=HDR, rdx=target & 0xFFFFFFFF, mem={HDR: hdr, BASE: bytes(data)})
    rax = out["rax"]
...
print(f"intrusive_map (stride 0x28, FUN_1404523c0): {total} cas ; {'BYTE-EXACT' if not fails else f'{fails} DIVERGENCES'}")
