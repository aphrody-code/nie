...
const cli = [...cliBlock.matchAll(/^    (?:#\[[^\n]+\]\n    )*([A-Z][A-Za-z0-9_]*)/gm)]
  ...
  .map(name => {
    ...
    return mapping("cli", name, cliOwners[name] ?? "crates/tools/nie-cli/src/main.rs", host ? "host_only" : "portable", host ? cliHostRationale[name] : `${name} delegates portable domain work to its named library owner.`);
  });
...
const inacord = registry.split(",").map(value => value.trim()).filter(value => /^[a-z_][a-z0-9_:]*$/.test(value))
  .map(name => {
    ...
    const mapped = mapping("inacord", name, nativeOwner(name), transport ? "transport" : host ? "host_only" : "portable", transport ? `${name} is a remote-service transport adapter.` : host ? `${name} requires native process, window, executable, or game-installation access.` : `${name} is an IPC adapter expected to delegate to the named portable owner.`);
    return leaf.startsWith("video_") ? { ...mapped, sourceEvidence: ["apps/inacord/src-tauri/src/video.rs", "nie_formats::usm"] } : mapped;
  });
