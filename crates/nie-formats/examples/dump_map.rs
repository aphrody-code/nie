...
fn main() {
    ...
    let mdf = pk
        .files
        .iter()
        .find(|f| f.name.ends_with(".g4md") || g4md::is_g4md(&pkm[f.offset..f.offset + f.size.min(4)]))
    ...
    for (i, sm) in md.submeshes.iter().enumerate() {
        println!("[{i}] v_off={} v_cnt={} stride={} idx_off={} idx_cnt={} layout={} mat={}",
            sm.vertex_offset, sm.vertex_count, sm.stride, sm.index_offset, sm.index_count,
            sm.layout_num, sm.material_index);
    }
    ...
    for (i, g) in geos.iter().enumerate() {
        ...
        for t in g.indices.chunks_exact(3) {
            ...
            if a.max(b).max(c) >= np { continue; }
            let p = |k: usize| g.positions[k];
            let d = |u: g4mg::Vec3, v: g4mg::Vec3| ((u.x-v.x).powi(2)+(u.y-v.y).powi(2)+(u.z-v.z).powi(2)).sqrt();
            edges.push(d(p(a), p(b)));
            edges.push(d(p(b), p(c)));
        }
        edges.sort_by(|a, b| a.partial_cmp(b).unwrap());
        let med = if edges.is_empty() { 0.0 } else { edges[edges.len()/2] };
        let mx = edges.last().copied().unwrap_or(0.0);
        let bad = edges.iter().filter(|&&e| e > 1e4).count();
        // bbox des positions
        let (mut lo, mut hi) = ([f32::MAX; 3], [f32::MIN; 3]);
        for p in &g.positions {
            for (k, v) in [p.x, p.y, p.z].iter().enumerate() { lo[k]=lo[k].min(*v); hi[k]=hi[k].max(*v); }
        }
        println!("[{i}] np={np} ni={ni} arête_médiane={med:.2} arête_max={mx:.2e} arêtes>1e4={bad}  bbox=({:.1},{:.1},{:.1})..({:.1},{:.1},{:.1})",
            lo[0],lo[1],lo[2],hi[0],hi[1],hi[2]);
        if i == 0 {
            println!("    pos[0..3]={:?}", g.positions.iter().take(3).map(|p|(p.x,p.y,p.z)).collect::<Vec<_>>());
            println!("    idx[0..9]={:?}", g.indices.iter().take(9).collect::<Vec<_>>());
        }
    }
}
