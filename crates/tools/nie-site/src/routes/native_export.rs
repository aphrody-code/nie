...
use axum::{Json, extract::{Path, Query, State}, http::{HeaderValue, header}, response::{IntoResponse, Response}};
...
pub async fn formats(State(state): State<EtatSite>, Path(raw): Path<String>) -> Result<Json<serde_json::Value>, ErreurSite> {
    ...
    Ok(Json(json!({ "path": path, "formats": formats, "selectionBasis": "original-file-extension",
...
pub async fn file(State(state): State<EtatSite>, Path(raw): Path<String>, Query(query): Query<ExportQuery>) -> Result<Response, ErreurSite> {
    let path = super::vfs::normaliser(&raw)?;
    let format = query.format.as_deref().unwrap_or("raw").to_owned();
    if !nie_explore::export::formats_pour(&path).iter().any(|candidate| candidate.id == format) {
        return Err(ErreurSite::Demande("Unsupported export format for this resource".into()));
    }
    if let Some(reason) = availability(&path, &format) {
        return Err(ErreurSite::Demande(reason.into()));
    }
    let size = state.index()?.taille(&path).ok_or_else(|| ErreurSite::Introuvable("VFS resource not found".into()))?;
    if size as usize > MAX_SOURCE_BYTES {
        return Err(ErreurSite::Demande("Resource exceeds the bounded conversion size; original bytes remain on the VFS route".into()));
    }
    let permit = Arc::clone(limiter()).try_acquire_owned()
    ...
    let output = tokio::task::spawn_blocking(move || {
        ...
        if bytes.len() > MAX_SOURCE_BYTES { return Err("Source exceeds conversion budget".to_owned()); }
        ...
        let output = if format == "wav" && lower.ends_with(".awb") {
            ...
            let channel = query.audio_channel.ok_or("An exact audioChannel is required")?;
            nie_explore::native_video::audio_track(&path, &bytes, channel)?
        } else if matches!(format.as_str(), "mp4" | "webm") && lower.ends_with(".usm") {
            let metadata: serde_json::Value = serde_json::from_str(&nie_explore::native_video::metadata(&path, &bytes)?)
                .map_err(|error| error.to_string())?;
            let expected = if format == "mp4" { "video/mp4" } else { "video/webm" };
    ...
    let safe_name: String = filename.chars().map(|c| if c.is_ascii_alphanumeric() || matches!(c, '.' | '_' | '-') { c } else { '_' }).collect();
