...
pub struct RelatedQuery { pub locale: Option<String> }

/// Declared references can feed preloading; naming candidates are not dependency proof.
pub async fn related(State(state): State<EtatSite>, Path(raw): Path<String>, Query(query): Query<RelatedQuery>) -> Result<Json<nie_explore::related::Relationships>, ErreurSite> {
    ...
    let permit = Arc::clone(LIMIT.get_or_init(|| Arc::new(tokio::sync::Semaphore::new(2))))
        .try_acquire_owned().map_err(|_| ErreurSite::Indisponible("Resource inspection capacity is busy".into()))?;
    let vfs = state.vfs()?;
