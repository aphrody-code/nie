...
/// Canonical English name for a seven-value matching profile.
pub type MatchingStats = StatsAppariement;

/// Canonical English name for the portable Zukan matching input.
pub type ZukanMatchEntry = EntreeZukan;

/// Canonical English name for a portable Inagle candidate row.
pub type InagleMatchCandidate = LigneInagle;

/// Maximum number of candidates accepted by [`rank_candidates_bounded`].
pub const MAX_MATCH_CANDIDATES: usize = 1_024;

/// Maximum number of ranked results returned by [`rank_candidates_bounded`].
pub const MAX_RANKED_RESULTS: usize = 64;

/// A scored candidate returned by the bounded portable matcher.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub struct RankedMatch {
    /// Stable candidate identifier from the Inagle row.
    pub candidate_id: String,
    /// Compatibility score calculated by the existing matcher.
    pub score: i32,
}

/// Bounds violation reported before the portable matcher allocates its result set.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub enum MatchLimitError {
    /// The caller supplied more candidates than the portable API accepts.
    ...
    /// The caller requested more output records than the portable API returns.
    #[error("requested result count {requested} exceeds maximum {maximum}")]
    TooManyResults {
        /// Result count requested by the caller.
        requested: usize,
        /// Supported result count.
        maximum: usize,
    },
}
...
/// Normalizes a position to the canonical English matching value.
#[must_use]
pub fn normalize_position(position: Option<&str>) -> Option<String> {
    norm_pos(position)
}
...
/// Normalizes an element to the canonical English matching value.
#[must_use]
pub fn normalize_element(element: Option<&str>) -> Option<String> {
    norm_elem(element)
}
...
/// Maps a Zukan game title to the canonical Inagle series name.
#[must_use]
pub fn game_to_series(game: Option<&str>) -> Option<String> {
    zukan_jeu_vers_serie(game)
}
...
/// Normalizes a Zukan gender to the compact Inagle representation.
#[must_use]
pub fn normalize_gender(gender: Option<&str>) -> Option<String> {
    norm_genre(gender)
}
...
/// Calculates the compatibility score through the canonical English API.
#[must_use]
pub fn matching_score(zukan: &ZukanMatchEntry, candidate: &InagleMatchCandidate) -> i32 {
    score_appariement(zukan, candidate)
}

/// Ranks compatible candidates with explicit input and output bounds.
///
/// Candidates scoring below [`SCORE_MINIMAL`] are omitted. Equal scores retain
/// their input order, making the result deterministic across native and Wasm
/// builds. A zero `max_results` value is valid and returns an empty list.
pub fn rank_candidates_bounded(
    zukan: &ZukanMatchEntry,
    candidates: &[InagleMatchCandidate],
    max_results: usize,
) -> Result<Vec<RankedMatch>, MatchLimitError> {
    if candidates.len() > MAX_MATCH_CANDIDATES {
        return Err(MatchLimitError::TooManyCandidates {
            actual: candidates.len(),
            maximum: MAX_MATCH_CANDIDATES,
        });
    }
    if max_results > MAX_RANKED_RESULTS {
        return Err(MatchLimitError::TooManyResults {
            requested: max_results,
            maximum: MAX_RANKED_RESULTS,
        });
    }
    ...
    let mut ranked: Vec<RankedMatch> = candidates
        ...
        .filter_map(|candidate| {
            let score = matching_score(zukan, candidate);
            (score >= SCORE_MINIMAL).then(|| RankedMatch {
...
mod tests {
    ...
    #[test]
    fn english_normalization_api_preserves_matching_rules() {
        assert_eq!(normalize_position(Some("Gardien")).as_deref(), Some("GK"));
        assert_eq!(normalize_element(Some("火")).as_deref(), Some("Fire"));
        assert_eq!(normalize_gender(Some("Female")).as_deref(), Some("F"));
        assert_eq!(
            game_to_series(Some("Inazuma Eleven GO Galaxy: Supernova")).as_deref(),
            Some("Galaxy")
        );
    }

    #[test]
    fn bounded_ranking_filters_sorts_and_keeps_ties_stable() {
        let mut z = zukan("Mark");
        z.position = Some("GK".to_string());
        z.element = Some("Wind".to_string());
        z.jeu = Some("Inazuma Eleven".to_string());

        let mut first = ligne("first");
        first.series = Some("Inazuma Eleven 2".to_string());
        let mut best = ligne("best");
        best.series = Some("Inazuma Eleven".to_string());
        let mut second = ligne("second");
        second.series = Some("Inazuma Eleven 2".to_string());
        let mut rejected = ligne("rejected");
        rejected.position = "FW".to_string();

        let ranked = rank_candidates_bounded(&z, &[first, best, second, rejected], 3).unwrap();
        assert_eq!(
            ranked,
            vec![
                RankedMatch {
                    candidate_id: "best".to_string(),
                    score: 70,
                },
                RankedMatch {
    ...
    #[test]
    fn bounded_ranking_rejects_oversized_requests() {
        let z = zukan("Mark");
        let candidates = vec![ligne("candidate"); MAX_MATCH_CANDIDATES + 1];
        assert_eq!(
            rank_candidates_bounded(&z, &candidates, 1),
            Err(MatchLimitError::TooManyCandidates {
                actual: MAX_MATCH_CANDIDATES + 1,
                maximum: MAX_MATCH_CANDIDATES,
            })
        );
        assert_eq!(
            rank_candidates_bounded(&z, &[], MAX_RANKED_RESULTS + 1),
            Err(MatchLimitError::TooManyResults {
                requested: MAX_RANKED_RESULTS + 1,
                maximum: MAX_RANKED_RESULTS,
            })
        );
    }
