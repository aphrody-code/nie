...
pub fn character(conn: &Connection, id: &str) -> anyhow::Result<Option<CharacterCard>> {
    let Some(profile) = crate::query::get_character(conn, id)? else { return Ok(None); };
    anyhow::ensure!(profile.skills.len() <= 128, "Character skill list exceeds its bounded join budget");
    ...
    for slot in profile.skills {
        ...
        learned_skills.push(LearnedSkill {
            skill_id: slot.skill_id, learn_level: slot.learn_level, resolved: skill.is_some(),
    ...
    Ok(Some(CharacterCard {
        character: CharaSummary { id: profile.id, chara_id: profile.chara_id,
            name_fr: profile.name_fr, name_en: profile.name_en, name_ja: profile.name_ja,
            element: profile.element, position: profile.position, rarity_label: profile.rarity_label,
            internal_code: profile.internal_code, slug: profile.slug, base_slug: profile.base_slug },
        description_fr: profile.description_fr, description_en: profile.description_en,
        team_name: profile.team_name, series: profile.series, zukan_hash: profile.zukan_hash,
        stats_lv1: profile.stats_lv1, stats_lv30: profile.stats_lv30,
        stats_lv50: profile.stats_lv50, stats_lv99: profile.stats_lv99,
        learned_skills, auras: profile.auras,
    }))
}
