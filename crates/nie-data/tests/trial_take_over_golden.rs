...
const PATH: &str =
    "/home/ubuntu/niers/data/common/gamedata/system/trial_take_over_config.cfg.bin.json";
...
fn comptes_et_valeurs_byte_exact() {
    ...
    assert!(!last.condition.is_empty(), "condition base64 non vide pour le dernier");
...
fn dispatch_typed_atteint_azalee() {
    ...
    let Some(root) = load() else { return };
    assert_eq!(family_key("trial_take_over_config.cfg.bin"), "trial_take_over_config");
    let (label, json) =
