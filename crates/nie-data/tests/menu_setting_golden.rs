...
fn main_menu_layers() -> [(i64, &'static str, &'static str, [i64; 7]); 13] {
    [
        (367379312, "mainmenu90_00_background", "mainmenu90_00_background.objbin", [1, 0, 0, 0, 0, 1, 1]),
        (-710450727, "mainmenu90_02_header_tab", "mainmenu90_02_header_tab.objbin", [1, 2, 4, 0, 0, 1, 1]),
        (200360024, "mainmenu90_02_2_header_tab_icon", "mainmenu90_02_2_header_tab_icon.objbin", [9, 2, 4, 0, 0, 1, 1]),
        (-1789734899, "cmn01_12_new_icon", "cmn01_12_new_icon.objbin", [9, 2, 4, 0, 0, 1, 1]),
        (1561203443, "cmn01_13_new_icon_red", "cmn01_13_new_icon_middle.objbin", [9, 2, 4, 0, 0, 1, 1]),
        (575058101, "mainmenu90_01_header", "mainmenu90_01_header.objbin", [1, 2, 4, 0, 0, 1, 1]),
        (-2015585191, "mainmenu90_31_doc_item", "mainmenu90_31_doc_item.objbin", [1, 2, 4, 0, 0, 1, 1]),
        (1209571854, "rpg00_07_weekday_timezone_guide", "rpg00_07_weekday_timezone_guide.objbin", [1, 0, 0, 0, 0, 1, 1]),
        (1138543975, "mainmenu01_06_base_button_guide", "mainmenu01_06_base_button_guide.objbin", [1, 2, 12, 0, 0, 1, 1]),
        (9396745, "mainmenu01_07_button_guide", "mainmenu01_07_button_guide.objbin", [10, 2, 12, 0, 0, 1, 1]),
        (2005383485, "mainmenu01_10_return_arrow_button_guide", "mainmenu01_10_return_arrow_button_guide.objbin", [1, 1, 12, 0, 0, 1, 1]),
        (-805989921, "mainmenu01_11_save_button_guide", "mainmenu01_11_save_button_guide.objbin", [1, 1, 12, 0, 0, 1, 1]),
        (-1212813856, "cmn01_40_list_base_empty", "cmn01_40_list_base_empty.objbin", [1, 0, 0, 0, 0, 1, 1]),
    ]
}
...
fn build_main_menu_setting() -> Value {
    ...
    let layers: Vec<Value> = main_menu_layers()
        .iter()
        .enumerate()
        .map(|(i, (id, name, obj, params))| {
            layer(i, *id, name, &format!("{OBJ}{obj}"), params)
        })
        .collect();
    let resources = vec![
    ...
    let commands = vec![
        cmd(0, 200360024, 1970176280, "CMD_FCS_BACK", &[-186917087, -186917087, 1]),
        cmd(1, 200360024, -1590706489, "CMD_FCS_NEXT", &[-186917087, -186917087, 1]),
        cmd(2, 200360024, -1283855071, "CMD_FUNCTION", &[-825442646, -186917087]),
        cmd(3, 200360024, -821903626, "CMD_FUNCTION", &[-825442646, -186917087, 1]),
    ];
...
fn main_menu_setting_parses_13_layers() {
    ...
    assert_eq!(ms.layers[0].objbin_path, "common/gamedata/menu/obj/mainmenu90_00_background.objbin");
    ...
    assert_eq!(ms.resources[0].logical_path, "#/menu/200_icon/100_num/num_menu01.g4tx");
}
...
fn layer_groups_mark_interactive_layer() {
    let ms = parse(&build_main_menu_setting());
    assert_eq!(ms.layer_groups.len(), 13, "13 MENU_LAYER_GROUP_BASE attendus");
    ...
    let interactive = ms.layer_groups.iter().filter(|g| g.flags.first() == Some(&1)).count();
    ...
    assert!(ms.commands.iter().all(|c| Some(c.layer_id) == ms.interactive_layer_id()));
}
