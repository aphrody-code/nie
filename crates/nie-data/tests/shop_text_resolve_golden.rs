...
use nie_data::shop::{resolve_name, ShopInfo};

fn shop(name_hash: u32) -> ShopInfo {
    ShopInfo { shop_id: HashId(0xB13B_2BD5), name_hash: HashId(name_hash), items: vec![] }
}
...
fn shop_text() -> Vec<(HashId, String)> {
    vec![
        ...
        (HashId(0x0000_1003), String::from("Centre commercial chronique")),
...
fn resolves_real_shop_names() {
    let t = shop_text();
    assert_eq!(resolve_name(&shop(0x0000_1001), &t), Some("Boutique Kizuna"));
    assert_eq!(resolve_name(&shop(0x0000_1002), &t), Some("BB Mart"));
    assert_eq!(resolve_name(&shop(0x0000_1003), &t), Some("Centre commercial chronique"));
}
...
fn duplicate_hash_last_wins() {
    assert_eq!(resolve_name(&shop(0x0000_2000), &shop_text()), Some("récent"));
}
