...
const CONDITION_SKILLS: &[(i64, u32)] = &[
    (1, 0x2CFC3E63), (1, 0x518BCA26), (1, 0x1ECA5CE1), (1, 0x1ECA5CE1), (1, 0x07D16DA0),
    (1, 0x63BDA8A4), (1, 0x518BCA26), (1, 0x4890FB67), (1, 0x518BCA26), (1, 0x4AD6453E),
    (1, 0x5F989B9E), (1, 0x19C3F43D), (1, 0x232D05EC), (1, 0x00D8C57C), (1, 0x19C3F43D),
    (1, 0x2354CA2D), (1, 0x7577A26A), (1, 0x09C23C18), (1, 0x111B676E), (1, 0xEDBAD1F1),
    (1, 0x9349F1DA), (1, 0x518BCA26), (1, 0x518BCA26), (3, 0x518BCA26), (1, 0x19C3F43D),
    (1, 0x19C3F43D), (1, 0xF4533922), (1, 0xBB12AFE5), (1, 0x8924CD67), (1, 0x65766A4F),
    (1, 0x8924CD67), (1, 0x7601AF96), (1, 0xC7A73197), (1, 0xB27F13ED), (1, 0x7DAF3139),
    (1, 0x69D15C3C), (1, 0x19C3F43D), (1, 0xED480863), (1, 0x2F78EA0D), (1, 0x518BCA26),
    (1, 0x83BAE06D), (1, 0xA897B3AE), (3, 0xB27F13ED), (1, 0x166B0073), (1, 0xDF70E04C),
    (3, 0x73C5AF40), (1, 0x6ADE9E01), (1, 0x142DBE2A), (2, 0x166B0073), (1, 0x7D6E9A87),
    (1, 0x8C382852), (4, 0x7D6E9A87), (2, 0x2B343D01), (3, 0x55C71D2A), (3, 0x8C382852),
    (1, 0x7CCC712C), (1, 0x0A02DF36), (1, 0x7601AF96), (1, 0x7F48A542), (1, 0x8E7E960B),
    (1, 0x7CCC712C),
];

// conditions : (conditionType, refStart, refCount) — n=33
const CONDITIONS: &[(i64, i64, i64)] = &[
    (2, 0, 2), (2, 2, 2), (2, 4, 2), (2, 6, 2), (2, 8, 2), (2, 10, 2), (2, 12, 2), (0, 14, 2),
    (0, 16, 3), (2, 19, 2), (2, 21, 2), (0, 23, 1), (2, 24, 2), (2, 26, 2), (2, 28, 2),
    (2, 30, 2), (2, 32, 2), (2, 34, 2), (2, 36, 2), (2, 38, 2), (2, 40, 2), (0, 42, 1),
    (0, 43, 2), (0, 45, 1), (0, 46, 2), (0, 48, 1), (0, 49, 2), (0, 51, 1), (0, 52, 1),
    (0, 53, 1), (0, 54, 1), (0, 55, 3), (0, 58, 3),
];

// overrides : (overrideSkillId hex, refStart, refCount) — n=33
const OVERRIDES: &[(u32, i64, i64)] = &[
    (0xBAA40F86, 0, 1), (0xD709B785, 1, 1), (0x2F997334, 2, 1), (0x5BF51EF0, 3, 1),
    (0xD9A58921, 4, 1), (0xED03D6F5, 5, 1), (0x2867AA8C, 6, 1), (0xE33B7929, 7, 1),
    (0xD78FD797, 8, 1), (0x4FA2C954, 9, 1), (0xC0E23CF4, 10, 1), (0x2753C574, 11, 1),
    (0x03E0C869, 12, 1), (0x8CA03DC9, 13, 1), (0x7430F978, 14, 1), (0x36758428, 15, 1),
    (0x3E1470A1, 16, 1), (0x040CE0BB, 17, 1), (0xCCEC6FCB, 18, 1), (0xFE364312, 19, 1),
    (0xC9E8B320, 20, 1), (0x5E2FCAC1, 21, 1), (0xCD309753, 22, 1), (0xB7F0C433, 23, 1),
    (0x687DA020, 24, 1), (0x3164B69D, 25, 1), (0x7205203E, 26, 1), (0x8D6B3D37, 27, 1),
    (0xB7F5CD3F, 28, 1), (0x450DD255, 29, 1), (0x552C2B5E, 30, 1), (0x15728836, 31, 1),
    (0x6FB2DB56, 32, 1),
];
...
fn condition_skills_valeurs_exactes() {
    ...
    for (i, (num, id)) in CONDITION_SKILLS.iter().enumerate() {
        ...
        assert_eq!(db.condition_skills[i].skill_id, HashId(*id), "skillId à l'index {i}");
    }
}
...
fn conditions_valeurs_exactes() {
    ...
    for (i, (ct, s, c)) in CONDITIONS.iter().enumerate() {
        assert_eq!(db.conditions[i].condition_type, *ct, "conditionType index {i}");
        assert_eq!(db.conditions[i].ref_condition_skill_data, [*s, *c], "ref index {i}");
    }
}
...
fn overrides_valeurs_exactes() {
    ...
    for (i, (id, s, c)) in OVERRIDES.iter().enumerate() {
        assert_eq!(db.overrides[i].override_skill_id, HashId(*id), "overrideSkillId index {i}");
        assert_eq!(db.overrides[i].ref_condition_data, [*s, *c], "ref index {i}");
    }
}
...
fn resolution_override8_condition_a_trois_skills() {
    ...
    assert_eq!(
        ids,
        vec![HashId(0x7577_A26A), HashId(0x09C2_3C18), HashId(0x111B_676E)]
    );
}
...
fn resolution_couvre_toutes_les_refs_sans_debordement() {
    ...
    for (oi, (_id, cs, cc)) in OVERRIDES.iter().enumerate() {
        ...
        assert_eq!(r.conditions.len(), expected_conds, "nb conditions override {oi}");
        for (k, ci) in (cond_start..cond_end).enumerate() {
            let (ct, ss, sc) = CONDITIONS[ci];
            assert_eq!(r.conditions[k].condition_type, ct, "type override {oi} cond {k}");
...
fn validation_contre_vrai_dump_si_present() {
    ...
    // Validé hors-ligne via crates/nie-game/examples/extract_override_skill.rs :
