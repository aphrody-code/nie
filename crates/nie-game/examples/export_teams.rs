...
fn main() {
    let out = std::env::args().nth(1).unwrap_or_else(|| "var/teams-resolved.json".into());
    ...
    for t in &teams {
        let Some(name) = resolve_team_name(t, &team_text) else { continue };
        list.push(json!({
    ...
    eprintln!("✓ export-teams: {} équipes résolues → {out} ({} octets)", doc["count"], txt.len());
}
