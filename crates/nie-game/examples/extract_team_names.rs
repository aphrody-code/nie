...
fn t2b_to_iecode(siblings: &[CfgEntry]) -> Vec<serde_json::Value> {
    ...
    siblings.iter().map(|e| {
        ...
        let name = format!("{}_{}", e.name, *idx); *idx += 1;
        let variables: Vec<serde_json::Value> = e.variables.iter().map(|v| match v {
...
fn rdbn_value_to_json(v: &RdbnValue) -> serde_json::Value {
    use RdbnValue as R;
    match v {
        R::Bool(b) => json!(b), R::Byte(n) => json!(n), R::Short(n) | R::ActType(n) => json!(n),
        R::Int(n) | R::Flag(n) => json!(n), R::Float(f) => json!(f),
        R::Hash(h) => json!(format!("0x{h:08X}")), R::Rates(a) | R::Position(a) => json!(a),
        R::Condition(s) => json!(s), R::ShortTuple(t) => json!(t), _ => serde_json::Value::Null,
    }
}
...
fn load_t2b(vfs: &Vfs, exact: &str, must: &str) -> serde_json::Value {
    let path = vfs.iter().map(|(p, _)| p.to_string()).filter(|p| p.contains(must) && p.rsplit('/').next() == Some(exact)).min().unwrap_or_else(|| panic!("{exact} introuvable"));
    eprintln!("  {path}");
...
fn load_t2b_prefix(vfs: &Vfs, prefix: &str, must: &str) -> serde_json::Value {
    let path = vfs.iter().map(|(p, _)| p.to_string()).filter(|p| p.contains(must) && p.rsplit('/').next().is_some_and(|b| b.starts_with(prefix) && b.ends_with(".cfg.bin"))).min().unwrap_or_else(|| panic!("{prefix} introuvable"));
    eprintln!("  {path}");
    let file = cfgbin::parse_t2b(&vfs.read(&path).expect("read")).expect("parse");
    json!({ "entries": t2b_to_iecode(&file.entries) })
}
...
fn main() {
    let dir = std::env::var("NIE_GAME_DIR").unwrap_or_else(|_| "/mnt/c/Program Files (x86)/Steam/steamapps/common/INAZUMA ELEVEN Victory Road".into());
    let mut vfs = Vfs::new();
    vfs.init(Path::new(&dir).join("data").as_path()).expect("vfs init");

    let teams = nie_data::belong_team::parse_belong_team_config(&load_rdbn(&vfs, "belong_team_config"));
    ...
    let named = teams.iter().filter(|t| resolve_team_name(t, &team_text).is_some()).count();
    eprintln!("noms d'équipe résolus = {named}/{}", teams.len());
    assert!(named > 0, "au moins un nom d'équipe résolu");
    for t in teams.iter().filter(|t| resolve_team_name(t, &team_text).is_some()).take(8) {
        eprintln!("  team {} → {:?}", t.belong_team_id.to_hex(), resolve_team_name(t, &team_text));
    }

    // Chaîne chara_base → belong_team : l'équipe d'Endou (c01000010, belong_team_id 0xF01BB293).
    let bases = nie_data::chara_base::parse_all_chara_base(&load_t2b_prefix(&vfs, "chara_base_1", "/character/"));
    use nie_data::hash::HashId;
    if let Some(endou) = nie_data::chara_base::find_by_chara_id(&bases, HashId(0x99A1_C150)) {
        let team = endou.belong_team_id.and_then(|id| nie_data::belong_team::find_by_id(&teams, id))
