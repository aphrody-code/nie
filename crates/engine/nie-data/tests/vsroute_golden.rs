...
use nie_data::vsroute::{
    parse_chronicle_vs_route_config, parse_opponent_info, parse_trigger,
};
...
fn cfg5_comptes_des_listes() {
    let Some(root) = load_json(REAL_CONFIG_5) else { return };
...
fn cfg5_gate_0() {
    ...
    assert_eq!(
        g.condition1,
        "AAAAACEFNcl4Pb8AEwIoAAYCNK0nb6ooAAYCMgAAAAIyAAAAAXg=",
...
#[test]
fn cfg2_comptes() {
    let Some(root) = load_json(REAL_CONFIG_2) else { return };
    let c = parse_chronicle_vs_route_config(&root);
...
fn opponent_compte_et_entree_0() {
    let Some(root) = load_json(REAL_OPPONENT) else { return };
    ...
    assert_eq!(o.cond1, "AAAAABgFNaNVuJ8ACgEoAAYCNEHkr7IyAAAAAXg=", "cond1 base64");
