...
fn real_sort_order() {
    ...
    // permutation de 0..269 : chaque index apparaît une fois
    let mut sorted = cfg.sort_order.clone();
    sorted.sort_unstable();
    let expect: std::vec::Vec<i64> = (0..269).collect();
    assert_eq!(sorted, expect, "__SORT_INDEX est une permutation de 0..269");
}
