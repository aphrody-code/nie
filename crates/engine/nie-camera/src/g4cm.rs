...
//!
//! ```text
//!   0x00  en-tete Level-5 commun (cf. nie_formats::level5)
...
pub struct CameraAnim {
    ...
    pub header: nie_formats::level5::Level5Header,
...
fn u16_at(d: &[u8], at: usize) -> Result<u16> {
    d.get(at..at + 2)
        ...
        .ok_or(CameraError::TooShort { got: d.len(), need: at + 2, at })
}

fn u32_at(d: &[u8], at: usize) -> Result<u32> {
    d.get(at..at + 4)
        .map(|b| u32::from_le_bytes([b[0], b[1], b[2], b[3]]))
        .ok_or(CameraError::TooShort { got: d.len(), need: at + 4, at })
}
...
///
/// # Errors
/// [`CameraError::BadMagic`] si le magic diffère, [`CameraError::UnsupportedVersion`] si le
/// conteneur n'est pas en version [`VERSION`], [`CameraError::TooShort`] /
/// [`CameraError::Malformed`] si une section sort du tampon.
pub fn decode(data: &[u8]) -> Result<CameraAnim> {
    let header = nie_formats::level5::parse_header(data, MAGIC, "G4CM").map_err(|e| match e {
        nie_formats::FormatError::BadMagic { .. } => CameraError::BadMagic { format: "G4CM" },
        other => CameraError::Format(other),
    })?;
    if header.type_id != VERSION {
        return Err(CameraError::UnsupportedVersion { got: header.type_id, expected: VERSION });
    }
    ...
    for i in 0..nobj {
        ...
        let mut tail = [0u8; 8];
        tail.copy_from_slice(data.get(at + 8..at + 16).ok_or(CameraError::TooShort {
            got: data.len(),
            need: at + 16,
            at,
        })?);
    ...
    for i in 0..nobj {
        ...
        let raw = data.get(at..).ok_or(CameraError::TooShort {
            got: data.len(),
            need: at + 1,
            at,
        })?;
        let end = raw.iter().position(|&b| b == 0).ok_or_else(|| {
            CameraError::Malformed(format!("nom d'objet non terminé à 0x{at:X}"))
        })?;
    ...
    for i in 0..total {
        ...
        let hdr = data.get(at..at + 8).ok_or(CameraError::TooShort {
            got: data.len(),
            need: at + 8,
            at,
        })?;
    ...
    for r in &raws {
        ...
        let track = match r.s1 {
            ...
            _ => {
                ...
                Track::Raw8(
                    data.get(base..end)
                        .ok_or(CameraError::TooShort { got: data.len(), need: end, at: base })?
                        .to_vec(),
                )
            }
        };
...
fn locate_values(data: &[u8], times_end: usize, values_len: usize) -> Result<usize> {
    ...
    Err(CameraError::Malformed(format!(
...
///
/// # Errors
/// [`CameraError::Malformed`] si les offsets recalculés se chevauchent (structure modifiée de
/// façon incohérente).
pub fn encode(anim: &CameraAnim) -> Result<Vec<u8>> {
    ...
    for c in &anim.channels {
        ...
        let count = u32::try_from(c.track.len()).map_err(|_| {
            CameraError::Malformed("canal de plus de 2^32 échantillons".to_string())
        })?;
...
mod tests {
    ...
    fn rejette_magic_et_version() {
        assert!(matches!(decode(&[0u8; 0x40]), Err(CameraError::BadMagic { .. })));
        let mut raw = synthetic();
        raw[6..8].copy_from_slice(&0x0066u16.to_le_bytes());
        assert!(matches!(decode(&raw), Err(CameraError::UnsupportedVersion { got: 0x66, .. })));
