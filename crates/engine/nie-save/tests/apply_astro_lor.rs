...
fn apply_and_verify_astro_lor_avatar() {
    ...
    let mut container =
        nie_save::parse(&raw_bytes, "002AB8F4-USERDATALIVE").expect("dechiffrement de la save");
    ...
    assert!(
        roster_data_pos + 24000 <= autosave_blob.body.len(),
        "debordement roster"
    );
    ...
    autosave_blob.body[slot12_pos..slot12_pos + 4].copy_from_slice(&ASTRO_LOR_VR_ID.to_le_bytes());
    ...
    if old_slot0 != 0 && old_slot0 != ASTRO_LOR_OG_ID && old_slot0 != ASTRO_LOR_VR_ID {
        ...
        autosave_blob.body[slot13_pos..slot13_pos + 4].copy_from_slice(&old_slot0.to_le_bytes());
    ...
    let steam_live_path = Path::new(
        r"C:\Program Files (x86)\Steam\userdata\1599409088\2799860\remote\002AB8F4-USERDATALIVE",
    );
    if steam_live_path.exists() {
        let steam_bak = Path::new(
            r"C:\Program Files (x86)\Steam\userdata\1599409088\2799860\remote\002AB8F4-USERDATALIVE.bak",
        );
        if !steam_bak.exists() {
        ...
        eprintln!(
            "Save live Steam mise a jour avec succes : {}",
            steam_live_path.display()
        );
