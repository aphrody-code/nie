...
pub fn legacy_search(vfs: &Vfs, needle: &str, limit: usize) -> Vec<(String, u32, String)> {
    let mut entries: Vec<_> = vfs.iter().filter(|(path, _)| path.contains(needle))
        .map(|(path, entry)| (path.to_owned(), entry.file_size, entry.cpk_filename.clone())).collect();
...
fn declared_reference(vfs: &Vfs, role: &'static str, logical: String, locale: &str) -> Reference {
    let normalized = logical.trim_start_matches('#').trim_start_matches('/').replace("<LG>", locale);
    let candidate = if normalized.starts_with("data/") { normalized } else { format!("data/{normalized}") };
    let entry = vfs.find(&candidate);
    Reference { role, logical_path: logical, source_bytes: entry.map(|entry| entry.file_size),
...
pub fn inspect(vfs: &Vfs, path: &str, locale: &str) -> Result<Relationships, String> {
    ...
    let mut result = Relationships { path: path.to_owned(), declared: vec![], naming_candidates: vec![], unresolved: vec![] };
    if path.ends_with(".objbin") {
        if entry.file_size > 1024 * 1024 { return Err("Object declaration exceeds its size budget".into()); }
        let bytes = vfs.read(path).map_err(|error| error.to_string())?;
        if bytes.len() > 1024 * 1024 { return Err("Object declaration exceeds its size budget".into()); }
        let object = nie_formats::objbin::parse(&bytes).map_err(|error| error.to_string())?;
        for (role, logical) in [("animationPackage", object.g4pkm_path), ("texture", object.g4tx_path),
            ("skeleton", object.skeleton_path), ("animation", object.anime_path)] {
            if let Some(logical) = logical { result.declared.push(declared_reference(vfs, role, logical, locale)); }
        }
        ...
        for (role, extension) in [("model", "g4md"), ("mesh", "g4mg"), ("texture", "g4tx")] {
            ...
            if candidate != path && vfs.find(&candidate).is_some() {
                result.naming_candidates.push(declared_reference(vfs, role, candidate, locale));
            }
        }
        let texture = format!("{stem}.g4tx").replace("/common/", "/dx11/");
        if vfs.find(&texture).is_some() && !result.naming_candidates.iter().any(|candidate| candidate.logical_path == texture) {
            result.naming_candidates.push(declared_reference(vfs, "texture", texture, locale));
        }
