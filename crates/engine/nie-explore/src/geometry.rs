//! Shared classification and decoding for the game's nine geometry families.
//!
//! Hosts provide bytes and optional companion metadata; this module owns format dispatch,
//! parsing, summaries, and complete JSON conversion without depending on HTTP or a VFS.

/// Geometry decoding failure independent of a transport.
#[derive(Debug)]
pub enum GeometryError {
    /// The requested shape or supplied bytes cannot be decoded as requested.
    InvalidRequest(String),
    /// A decoded structure could not be represented as JSON.
    Internal(String),
}
...
impl Forme {
    ...
    pub fn depuis(s: Option<&str>) -> Result<Self, GeometryError> {
        match s.map(str::trim).filter(|f| !f.is_empty()) {
            None | Some("resume") => Ok(Self::Resume),
            Some("complet") => Ok(Self::Complet),
            Some(autre) => Err(GeometryError::InvalidRequest(format!(
...
fn illisible(famille: Famille, e: &impl std::fmt::Display) -> GeometryError {
    GeometryError::InvalidRequest(format!("{} illisible: {e}", famille.jeton().to_uppercase()))
}
    ...
    let resume = match famille {
        Famille::G4mg => {
            ...
            let md = g4md::parse(&c.octets)
                .map_err(|e| GeometryError::InvalidRequest(format!("G4MD illisible: {e}")))?;
...
fn en_valeur<T: Serialize>(v: &T) -> Result<serde_json::Value, GeometryError> {
    serde_json::to_value(v)
        .map_err(|e| GeometryError::Internal(format!("structure non serialisable: {e}")))
}
...
mod tests {
    ...
    fn chaque_famille_refuse_des_octets_qui_ne_sont_pas_les_siens() {
        for (suffixe, famille, _) in FAMILLES {
            ...
            .unwrap_err();
            assert!(matches!(e, GeometryError::InvalidRequest(_)), "{suffixe}");
        }
    }
    ...
    fn les_deux_formes_sont_reconnues_et_les_autres_refusees() {
        ...
        assert!(matches!(e, GeometryError::InvalidRequest(_)));
    }
