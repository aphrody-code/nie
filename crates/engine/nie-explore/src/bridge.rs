...
fn json_to_t2b_value(v: &Value) -> Result<T2bValue, String> {
    let ty = v.get("type").and_then(Value::as_str).ok_or("variable sans champ \"type\"")?;
    ...
    let raw = v.get("value").and_then(Value::as_str).ok_or("variable sans champ \"value\" (chaîne)")?;
    match ty {
        "String" => Ok(T2bValue::String(raw.to_string())),
        "Int" => raw.parse::<i32>().map(T2bValue::Int).map_err(|e| format!("valeur Int invalide {raw:?} : {e}")),
        "Float" => raw.parse::<f32>().map(T2bValue::Float).map_err(|e| format!("valeur Float invalide {raw:?} : {e}")),
        other => Err(format!("type de variable inconnu : {other:?} (attendu String/Int/Float)")),
    }
}

fn json_to_t2b_entry(v: &Value) -> Result<CfgEntry, String> {
    let name = v.get("name").and_then(Value::as_str).ok_or("entrée sans champ \"name\"")?.to_string();
    let variables = v
        .get("variables")
        .and_then(Value::as_array)
        .map(|a| a.iter().map(json_to_t2b_value).collect::<Result<Vec<_>, _>>())
        .transpose()?
        .unwrap_or_default();
    let children = v
        .get("children")
        .and_then(Value::as_array)
        .map(|a| a.iter().map(json_to_t2b_entry).collect::<Result<Vec<_>, _>>())
        .transpose()?
        .unwrap_or_default();
    Ok(CfgEntry { name, variables, children })
}
...
pub fn json_to_t2b_entries(root: &Value) -> Result<Vec<CfgEntry>, String> {
    let entries = root.get("entries").and_then(Value::as_array).ok_or("JSON sans champ \"entries\" (forme T2B attendue)")?;
...
fn patch_rdbn_value(original: &RdbnValue, edited: &Value, field_name: &str) -> Result<RdbnValue, String> {
    let err = || format!("champ {field_name:?} : valeur JSON incompatible avec son type d'origine ({original:?})");
    match original {
        ...
        RdbnValue::Flag(_) => edited
            ...
            .map(RdbnValue::Flag)
            .ok_or_else(err),
        RdbnValue::Float(_) => edited.as_f64().map(|f| RdbnValue::Float(f as f32)).ok_or_else(err),
        ...
        RdbnValue::Rates(_) | RdbnValue::Position(_) => {
            ...
            Ok(if matches!(original, RdbnValue::Rates(_)) { RdbnValue::Rates(out) } else { RdbnValue::Position(out) })
        }
        RdbnValue::Condition(_) => edited.as_str().map(|s| RdbnValue::Condition(s.to_string())).ok_or_else(err),
        RdbnValue::ShortTuple(_) => {
            let arr = edited.as_array().filter(|a| a.len() == 2).ok_or_else(err)?;
            let a = arr[0].as_i64().and_then(|n| i16::try_from(n).ok()).ok_or_else(err)?;
            let b = arr[1].as_i64().and_then(|n| i16::try_from(n).ok()).ok_or_else(err)?;
...
fn patch_rdbn_row(original: &RdbnRow, edited: &Value, list_name: &str, row_index: usize) -> Result<RdbnRow, String> {
    let map = edited
        .as_object()
        .ok_or_else(|| format!("liste {list_name:?} ligne {row_index} : JSON attendu (objet), pas {edited}"))?;
    ...
    for (name, orig_value) in &original.fields {
        let edited_value = map
            .get(name)
            .ok_or_else(|| format!("liste {list_name:?} ligne {row_index} : champ {name:?} absent du JSON édité"))?;
        fields.push((name.clone(), patch_rdbn_value(orig_value, edited_value, name)?));
    }
    Ok(RdbnRow { fields })
}
...
fn patch_rdbn_list(original: &RdbnList, edited: &Value) -> Result<RdbnList, String> {
    let rows_json = edited
        .get("values")
        .and_then(Value::as_array)
        .ok_or_else(|| format!("liste {:?} : JSON édité sans champ \"values\" (tableau)", original.name))?;
    ...
    Ok(RdbnList { name: original.name.clone(), type_name: original.type_name.clone(), rows })
}
...
pub fn json_to_rdbn_lists(original: &[RdbnList], edited: &Value) -> Result<Vec<RdbnList>, String> {
    let lists_json = edited.get("lists").and_then(Value::as_array).ok_or("JSON sans champ \"lists\" (forme RDBN attendue)")?;
...
mod tests {
    ...
    fn json_bridge_round_trip_sur_le_vrai_jeu() {
        ...
        for path in candidates.iter().step_by(step) {
            ...
            let Ok(original) = parse_t2b(&bytes) else { continue };
            n_t2b += 1;
            ...
            match parse_t2b(&reencoded) {
                ...
                Ok(_) => failed.push((path.clone(), "arbre différent après round-trip JSON".to_string())),
                Err(e) => failed.push((path.clone(), format!("redécodage échoué : {e}"))),
            }
        }

        eprintln!("json_bridge round-trip : {n_ok}/{n_t2b} identiques (sur {} candidats, pas={step})", candidates.len());
        for (p, e) in failed.iter().take(10) {
            eprintln!("  échec {p} : {e}");
        }
        assert!(n_t2b > 5, "attendu au moins quelques fichiers T2B réels, obtenu {n_t2b}");
        assert_eq!(n_ok, n_t2b, "{} échec(s) sur {n_t2b} fichiers T2B réels", failed.len());
    }
    ...
    fn json_bridge_rdbn_round_trip_sur_le_vrai_jeu() {
            ...
            match parse(&reencoded) {
                Ok(rdbn2) => {
                    ...
                    if roundtripped == original {
                        ...
                        failed.push((path.clone(), "listes différentes après round-trip JSON".to_string()));
                    }
                }
                Err(e) => failed.push((path.clone(), format!("redécodage échoué : {e}"))),
            }
        ...
        eprintln!("json_bridge_rdbn round-trip : {n_ok}/{n_rdbn} identiques (sur {} candidats, pas={step})", candidates.len());
        for (p, e) in failed.iter().take(10) {
            eprintln!("  échec {p} : {e}");
        }
        assert!(n_rdbn > 5, "attendu au moins quelques fichiers RDBN réels, obtenu {n_rdbn}");
        assert_eq!(n_ok, n_rdbn, "{} échec(s) sur {n_rdbn} fichiers RDBN réels", failed.len());
    }
}
