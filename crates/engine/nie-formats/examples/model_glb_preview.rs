...
//! géométrie doit rester exportable sans texture plutôt qu'échouer.
...
fn main() {
    ...
    let g4tx = if a[3] == "-" {
        ...
        Some(std::fs::read(&a[3]).expect("lecture g4tx"))
    };
    let out = &a[4];
    ...
    if let Some(png) = g4tx.as_ref().and_then(|tx| {
        ...
        eprintln!("(aucune texture embarquée : {})", a[3]);
    }
