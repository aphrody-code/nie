...
fn main() -> Result<(), String> {
    ...
    println!("modele charge en {:?} : {} primitives, {tris} triangles", t0.elapsed(), modele.primitives.len());
    ...
    let mut out = Vec::new();
    {
        let mut e = png::Encoder::new(std::io::Cursor::new(&mut out), nie_app::W as u32, nie_app::H as u32);
        e.set_color(png::ColorType::Rgba);
        e.set_depth(png::BitDepth::Eight);
        e.write_header().map_err(|e| e.to_string())?.write_image_data(&px).map_err(|e| e.to_string())?;
    }
