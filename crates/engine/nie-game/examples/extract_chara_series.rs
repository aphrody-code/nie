...
fn main() {
    ...
    let series = nie_data::chara_series::parse_chara_series_config(&load_rdbn(&vfs, "chara_series_config", "/character/"));
    eprintln!("[chara_series] séries = {}", series.len());
    assert_eq!(series.len(), 9, "9 séries (probe live)");
    for s in &series {
        eprintln!("  série {} type={} → {:?}", s.series_id.to_hex(), s.series_type, nie_data::chara_series::series_name_fr(s.series_type));
    }

    let bases = nie_data::chara_base::parse_all_chara_base(&load_t2b(&vfs, "chara_base_1", "/character/"));
    ...
    eprintln!("Endou (c01000010, series_id={:?}) → série = {endou_series:?}", endou.series_id.map(|h| h.to_hex()));
    assert_eq!(endou_series, Some("Inazuma Eleven"), "Endou = série 1 (IE original)");
    ...
    eprintln!("✓ END-TO-END OK : 9 séries + résolution chara_base.series_id → nom (Endou = Inazuma Eleven)");
}
