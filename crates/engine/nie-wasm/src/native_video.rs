...
fn metadata(original_name: &str, bytes: &[u8]) -> Result<String, String> {
...
fn video_track(original_name: &str, bytes: &[u8]) -> Result<Vec<u8>, String> {
...
fn audio_track(original_name: &str, bytes: &[u8], channel: u8) -> Result<Vec<u8>, String> {
    let movie = demux(original_name, bytes)?;
...
fn elementary_video(original_name: &str, bytes: &[u8]) -> Result<Vec<u8>, String> {
    let movie = demux(original_name, bytes)?;
