...
fn le_jeu_complet_est_produit_et_chaque_png_a_sa_taille() {
    ...
    let fichiers = p
        .brand_assets(frame, TAILLES_FAVICON)
        .expect("assets produits");

    for t in TAILLES_FAVICON {
        let f = fichiers
            .iter()
            .find(|f| f.nom == format!("icon-{t}.png"))
            .unwrap_or_else(|| panic!("icon-{t}.png manquante"));
        ...
        assert_eq!((w, h), (*t, *t), "icon-{t}.png annonce {w}×{h}");
    }

    for nom in ["favicon.ico", "icon.svg", "site.webmanifest"] {
...
fn l_ico_declare_ses_entrees_avec_des_offsets_qui_tombent_juste() {
    let p = pet();
    let frame = &p.animation("idle").expect("idle").frames[0];
    let fichiers = p.brand_assets(frame, TAILLES_FAVICON).expect("assets");
    let ico = &fichiers
        .iter()
...
fn le_manifeste_porte_les_deux_tailles_exigees_par_une_app_installable() {
    let p = pet();
    let m = p.web_manifest(TAILLES_FAVICON);
    ...
    assert!(
        icones.iter().all(|icon| icon["src"]
            .as_str()
            .is_some_and(|source| source.starts_with("/static/icon-"))),
        "every generated icon URL uses the English canonical name"
    );
}
...
fn le_svg_reste_leger_et_bien_forme() {
    ...
    let frame = &p.animation("idle").expect("idle").frames[0];
    let fichiers = p.brand_assets(frame, TAILLES_FAVICON).expect("assets");
    let svg = fichiers.iter().find(|f| f.nom == "icon.svg").expect("svg");
    assert!(
