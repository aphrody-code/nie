...
import * as game from "./url-conventions.ts";

const SERVEUR = readFileSync(
	join(resolve(import.meta.dir, "../../.."), "crates", "tools", "nie-model-serve", "src", "main.rs"),
	"utf8",
);
...
describe("la forme des URL", () => {
	const B = game.baseJeu();

	test("le `.g4tx` se retire pour `/tex`, quelle qu'en soit la casse", () => {
		expect(game.cheminTexture("data/dx11/chr/x.g4tx")).toBe("/tex/data/dx11/chr/x.png");
		expect(game.cheminTexture("data/dx11/chr/x.G4TX")).toBe("/tex/data/dx11/chr/x.png");
		expect(game.cheminTexture("data/dx11/chr/x")).toBe("/tex/data/dx11/chr/x.png");
	});

	test("une texture nommée garde le `.g4tx` du conteneur", () => {
		expect(game.cheminTextureNommee("dx11/menu/icon_item01.g4tx", "icon_a")).toBe(
			"/tex/dx11/menu/icon_item01.g4tx/icon_a.png",
		);
	});

	test("`/vfs/*` prend son chemin en query, jamais en segment", () => {
		expect(game.cheminFiche("data/a b/c.bin")).toBe("/vfs/stat?path=data%2Fa+b%2Fc.bin");
		expect(game.cheminListe("data/dx11/menu")).toBe("/vfs/ls?path=data%2Fdx11%2Fmenu&limit=500");
		expect(game.cheminRecherche("mark", { ext: "g4tx", limite: 20 })).toBe(
			"/vfs/find?q=mark&limit=20&ext=g4tx",
		);
		expect(game.cheminRecherche("mark")).toBe("/vfs/find?q=mark&limit=100");
	});
	...
	test("le décalage n'entre dans l'URL que lorsqu'on le demande", () => {
		...
		expect(game.cheminListe("data/dx11/menu", 1000, 0)).toBe(
			"/vfs/ls?path=data%2Fdx11%2Fmenu&limit=1000&offset=0",
		);
		expect(game.cheminRecherche("mark", { limite: 200, decalage: 0 })).toBe(
			"/vfs/find?q=mark&limit=200&offset=0",
		);
		expect(game.cheminRecherche("mark", { limite: 200, decalage: 40, ext: "usm" })).toBe(
	...
	test("`/dx11/` retire `data/dx11/` comme `data/`, et porte la variante", () => {
		...
		expect(game.cheminDx11("data/dx11/menu/a/b.g4tx")).toBe("/dx11/menu/a/b.png");
		expect(game.cheminDx11("data/common/x.g4tx")).toBe("/dx11/common/x.png");
		expect(game.cheminDx11("data/dx11/menu/a/b.g4tx", { largeur: 400 })).toBe(
	...
	test("une image déjà nommée par le serveur n'est pas reconstruite", () => {
		...
		expect(game.cheminImage(servi)).toBe(servi);
		expect(game.cheminImage(servi, { largeur: 160 })).toBe(`${servi}?w=160&format=webp`);
	});

	test("`/export` porte le format en query et l'identifiant de sous-entité avec", () => {
		expect(game.cheminExport("data/common/a.acb", "wav")).toBe(
			"/export/data/common/a.acb?format=wav",
		);
		expect(game.cheminExport("data/common/a.acb", "wav", 12)).toBe(
			"/export/data/common/a.acb?format=wav&id=12",
		);
	});

	test("`/audio` n'ajoute `?id=` que lorsqu'un cue est visé", () => {
		expect(game.cheminAudio("data/x.acb")).toBe("/audio/data/x.acb");
		expect(game.cheminAudio("data/x.acb", 0)).toBe("/audio/data/x.acb?id=0");
		expect(game.cheminAudio("data/x.acb", null)).toBe("/audio/data/x.acb");
	});

	test("une URL absolue est sa base suivie de son chemin", () => {
		expect(game.urlFichier("data/a.bin")).toBe(`${B}/raw/data/a.bin`);
		expect(game.urlFilm("common/movie/ev01_00050.usm")).toBe(
			`${B}/video/common/movie/ev01_00050.usm`,
		);
		expect(game.urlBandeSon("common/movie/ev01_00050.usm")).toBe(
			`${B}/video/common/movie/ev01_00050.usm?track=audio`,
		);
		expect(game.urlCatalogueFilms()).toBe(`${B}/video/catalog.json`);
		expect(game.urlModeleComplet("c01000010")).toBe(`${B}/model-full/c01000010.glb`);
		expect(game.urlModeleChr("waza", "ev60_00340")).toBe(`${B}/model-chr/waza/ev60_00340.glb`);
	});
	...
	test("un écran menu utilise la route typed et son suffixe canonique", () => {
		expect(game.menuSettingPath("main_menu")).toBe(
			"/typed/data/common/gamedata/menu/cfg/main_menu_setting.cfg.bin.json",
		);
		expect(game.menuSettingUrl("main_menu")).toBe(
	...
	test("un écran menu utilise aussi la route publique de nie-site", () => {
		expect(game.menuSettingApiPath("main_menu")).toBe("/api/v1/menu/settings/main_menu");
		expect(game.menuSettingApiPath("screen with/slash")).toBe(
