 ...
 *   5. rendu des routes témoins actuellement publiées par le site.
 *
...
/** Force the canonical source runner; standalone builds can rename CDP callback identifiers. */
const BXC_RUN_OPTIONS = { env: { ...process.env, BXC_FROM_SOURCE: "1" } };
...
const dynamicPages = (characterDetailPath: string) => [
  { name: "character-detail", path: characterDetailPath },
  ...
  {
    name: "news-detail",
    path: "/news/critique-communautaire-de-inazuma-eleven-victory-road",
  },
  ...
  program
    ...
    .action(async (options: TestOptions) => {
      if (options.playwright) {
        console.log(
          `${colors.cyan}Lancement des tests Playwright E2E...${colors.reset}`,
        );
      ...
      console.log(
        `${colors.bold}${colors.magenta}=== RUNNING NATIVE VERIFICATION SUITE ===${colors.reset}\n`,
      );

      // ─── SUITE 1 : filtres de base de données ───
      console.log(
        `${colors.bold}${colors.blue}SUITE 1: FILTRES DE BASE DE DONNÉES (SQLITE)${colors.reset}`,
      );
      const dbPath = getSqlitePath();
      if (!dbPath) {
        console.error(
          `${colors.red}Erreur: Base de données SQLite introuvable pour les tests.${colors.reset}`,
        );
        process.exit(1);
      }

      let characterDetailPath = "";
      try {
        const db = openReadonlyDatabase(dbPath);
        const characterWitness = db
          .query(
            "SELECT slug FROM inagle_characters WHERE slug IS NOT NULL AND slug <> '' ORDER BY zukan_order, slug LIMIT 1",
          )
          .get() as { slug?: string } | null;
        if (!characterWitness?.slug) {
          throw new Error(
            "No published character slug is available in the SQLite mirror",
          );
        }
        characterDetailPath = `/chara/${encodeURIComponent(characterWitness.slug)}`;
        ...
        const runTestStep = (
          ...
          try {
            const rows = db
              .query(queryStr)
              .all(...(params as never[])) as Array<{ count?: number }>;
            const count =
              rows.length > 0 && rows[0].count !== undefined
                ? rows[0].count
                : rows.length;
      ...
      try {
        if (!(await isServerRunning())) {
          ...
          const BOOT_TIMEOUT_S = Number.parseInt(
            process.env.AZALEE_DEV_BOOT_TIMEOUT ?? "180",
            10,
          );
          let ready = false;
          ...
          console.log(
            `${colors.green}Serveur local démarré et prêt!${colors.reset}\n`,
          );
        } else {
          console.log(
            `${colors.green}Serveur local existant détecté sur http://localhost:3000.${colors.reset}\n`,
          );
        }

        // ─── SUITE 2 : endpoints API & GraphQL ───
        console.log(
          `${colors.bold}${colors.blue}SUITE 2: API & GRAPHQL ENDPOINTS (HTTP)${colors.reset}`,
        );
        ...
        // 2. GET /api/news/feed
        {
          ...
          if (body.items.length > 0) {
            ...
            if (
              ...
              throw new Error(
                `Propriétés d'article invalides : ${JSON.stringify(item)}`,
              );
            }
          }
          console.log(
        ...
        // 3. GET /api/tags/popular
        {
          ...
          if (!Array.isArray(body))
            throw new Error("Le corps de la réponse n'est pas un tableau");
          if (body.length > 0) {
            if (body[0].tag === undefined || body[0].count === undefined) {
              throw new Error(
                `Structure de tag invalide : ${JSON.stringify(body[0])}`,
              );
            }
          }
          console.log(
        ...
        // 4. POST /api/graphql
        {
          ...
          if (
            ...
            throw new Error(
              `Réponse GraphQL invalide : ${JSON.stringify(body)}`,
            );
          }
          const char = body.data.characters[0];
          if (!char.id || !char.name || !char.name.fr) {
            throw new Error(
              `Objet personnage GraphQL invalide : ${JSON.stringify(char)}`,
            );
          }
          console.log(
        ...
        const jsonDeBxc = (sortie: string): unknown => {
          ...
          if (debut < 0)
            throw new Error(`sortie bxc sans JSON : ${sortie.slice(0, 120)}`);
          return JSON.parse(sortie.slice(debut));
        };

        // ─── SUITE 3 : audits du moteur bxc ───
        console.log(
          `${colors.bold}${colors.blue}SUITE 3: BXC BROWSER ENGINE AUDITS (CLI)${colors.reset}`,
        );
        ...
        // 1. bxc detect
        {
          ...
          if (data.httpStatus !== 200)
            throw new Error(`Bxc httpStatus: ${data.httpStatus}`);
          if (data.hostname !== "localhost")
            throw new Error(`Bxc hostname: ${data.hostname}`);

          const nextJsDetect = data.frontend?.find(
            (tech: any) => tech.name === "Next.js",
          );
          if (!nextJsDetect) throw new Error("Next.js non détecté par Bxc");
          if (nextJsDetect.confidence <= 0.5)
            throw new Error(
              `Confiance de détection Next.js trop faible : ${nextJsDetect.confidence}`,
            );
          console.log(
        ...
        // 2. bxc recon page d'accueil
        {
          ...
          if (!data.bytes || data.bytes <= 0)
            throw new Error(`Taille de page invalide : ${data.bytes}`);
          ...
          if (
            ...
            !data.finalUrl.startsWith("http")
          )
            throw new Error(
              `URL finale invalide : ${JSON.stringify(data.finalUrl)}`,
            );
          if (!Array.isArray(data.assets) || data.assets.length === 0)
            throw new Error("Aucun asset détecté");
          if (
            !Array.isArray(data.cssSelectors) ||
            data.cssSelectors.length === 0
          )
        ...
        console.log(
          `${colors.bold}${colors.blue}SUITE 4: INTERACTIVE PAGE ELEMENTS (DOM SURVEY)${colors.reset}`,
        );
        ...
        // 1. Bouton de bascule de thème
        {
          ...
          if (!html.includes("lucide-sun") && !html.includes("lucide-moon")) {
            throw new Error(
              "Bouton theme toggle (lucide-sun ou moon) introuvable dans le HTML",
            );
          }
          console.log(
        ...
        // 2. Filtres de la liste de personnages
        {
          ...
          if (!hasFw && !hasGk) {
            throw new Error(
              "Filtres de poste ('Attaquant' ou 'FW' ou 'Gardien') introuvables",
            );
          }
          console.log(
        ...
        {
          const start = performance.now();
          const res = await fetch(`${url}${characterDetailPath}`);
          const html = await res.text();
          const duration = (performance.now() - start).toFixed(1);
          const hasTabs =
            html.includes('role="tab"') || html.includes("role='tab'");
          if (hasTabs) {
            console.log(
        ...
        const allPages = [
          ...STATIC_PAGES,
          ...dynamicPages(characterDetailPath),
        ];
        console.log(
          `${colors.bold}${colors.blue}SUITE 5: PAGE RENDER VERIFICATION (${allPages.length} ROUTES)${colors.reset}`,
        );
        ...
        console.log(
          `${colors.bold}${colors.green}🎉 TOUTES LES SUITES DE TESTS NATIVES ONT RÉUSSI AVEC SUCCÈS !${colors.reset}`,
        );
        console.log(
          `${colors.bold}${colors.green}================================================================${colors.reset}\n`,
        );
