/**
 * **Synergy** — ce que ce paquet apporte et qu'aucun des quatre gisements ne porte seul.
 *
...
export async function fichiersDe(identifiant: string, limite = 100): Promise<string[]> {
	try {
		const reponse = await fetch(game.urlRecherche(identifiant, { limite }), {
...
export function personnage(slug: string): FichePersonnage | null {
	const fiche = excerpt.personnage(slug);
	if (!fiche) {
		return null;
	}
	...
	return {
		...
		media: {
			modele: fichiers.modeles[0] ? game.urlFichier(fichiers.modeles[0]) : null,
			portrait: fichiers.textures[0] ? game.urlTexture(fichiers.textures[0]) : null,
			voix: fichiers.sons.find((s) => s.endsWith(".acb"))
				? game.urlBanqueSon(fichiers.sons.find((s) => s.endsWith(".acb")) as string)
				: null,
		},
	};
}
...
export async function personnageComplet(slug: string): Promise<FichePersonnage | null> {
	...
	return {
		...
		media: {
			modele: fichiers.modeles[0] ? game.urlFichier(fichiers.modeles[0]) : null,
			portrait: fichiers.textures[0] ? game.urlTexture(fichiers.textures[0]) : null,
			voix: acb ? game.urlBanqueSon(acb) : null,
		},
	};
}
...
export function film(nom: string): FicheFilm {
	...
	const evenement = excerpt.ligne<Record<string, unknown>>(
		"SELECT * FROM inagle_events WHERE event_id = ? LIMIT 1",
		[nom],
	);
	return {
		bandeSon: game.urlBandeSon(cheminCommun),
...
export function technique(id: string): FicheTechnique | null {
	const fiche = excerpt.technique(id);
	if (!fiche) {
		return null;
	}
	return {
		...
		telop: {
			confiance: "cle",
			par: fiche.id,
			valeur: excerpt.ligne("SELECT * FROM inagle_telop_waza WHERE skill_id = ? LIMIT 1", [
				fiche.id,
			]),
		},
		videos: {
...
export interface Resultats {
	personnages: excerpt.Personnage[];
	episodes: anime.Episode[];
	fonctions: re.Fonction[];
	fichiers: excerpt.AssetJeu[];
}
...
export function chercher(texte: string, limite = 10): Resultats {
	return {
		...
		fichiers: excerpt.assets(texte, limite),
		fonctions: re.fonctions(texte, limite),
		personnages: excerpt.chercherPersonnages(texte, limite),
	};
}
