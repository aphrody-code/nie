...
export function useResolvedNames(resolver: NameResolver, source: string, locale: Locale, codes: string[]) {
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(() => resolver(source, missing, locale)).then(settle, () => settle(new Map()));
  }, [current, resolver, source, locale, signature]);
