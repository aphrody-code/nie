...
export interface ParseResult {
  ...
  validationGate?: string | undefined;
}
  ...
  static parseCode(filepath: string, content: string): ParseResult {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      switch (lang.id) {
        ...
        case "lua": {
          const reqMatch = line.match(/(?:require|INCLUDE)\s*\(\s*["'](.*?)["']\s*\)/);
          if (reqMatch?.[1]) imports.push(reqMatch[1]);
          const fnMatch = line.match(/^(?:local\s+)?function\s+([a-zA-Z0-9_.:]+)/);
          if (fnMatch?.[1]) functions.push(fnMatch[1]);
          break;
        }
      }
