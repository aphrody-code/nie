...
- **Constat VFS (2026-06-13, mesuré)** : `Vfs::init()` ne peut **pas** exploiter `cpk_list.cfg.bin` sur cette install — le fichier (12,9 Mo) reste illisible **après** `decrypt_block(VIOLA_FIXED_KEY)` (clé pourtant correcte) : il est chiffré **et compressé** autrement → tâche RE dédiée ouverte. **Sans impact correctness** : l'accès aux assets passe par le TOC `@UTF` par-CPK (921/921 déchiffrent, 250 800 fichiers indexés) ; `cpk_list.cfg.bin` ne serait qu'un index prêt-à-l'emploi (confort, pas une porte).
...
- **Reste** : bump wgpu **22→29** + retarget du port D3D11 `nie-engine/render.rs` vers `wgpu::Surface` + gate pixel-diff (SSIM ≥ 0,99 + égalité octet). Stack complète : `docs/STACK.md`. Inventaire : `docs/INVENTAIRE.md`.
