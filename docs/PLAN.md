...
- **FAIT (2026-06-10) : 3 familles de match portées → 7/58** — `formation` (`formation.rs` : 10 positions, 1073 placements, 115 formations ; positions terrain = 16 hex-chars = 2 f32 LE, ex. `"000000008FC2753F"` → (0.0, 0.9599) GK profond ; 21 tests), `command` (`command.rs` : effets de commande soccer/rpg, format à noeuds positionnels `entries`, 32 tests), `ai` (`ai.rs` : IA stratégie/tactique, 5 dumps format `lists`, 73 tests). Tous **golden byte sur les vrais dumps** de `data/common/gamedata/`. Ces familles nourrissent directement la boucle de match (cf. §3).
