...
| `.lua.bin` (bytecode Lua) | `bytecode::Chunk`/`Prototype`/`decode_instruction` | `crates/engine/nie-lua/src/bytecode.rs:56` (`Header`), `:133` (`Prototype`), `:183` (`Chunk`), `:282` (`decode_instruction`) — 34 `pub fn` exposées (mesure historique du 2026-09-06) |
