...
- **Constat VFS mesuré (2026-06-13)** : même corrigé, **`Vfs::init()` ne peut pas exploiter `cpk_list.cfg.bin`** sur cette install Steam. Le fichier (12,9 Mo) reste illisible après `decrypt_block` avec `VIOLA_FIXED_KEY` (= `key_from_filename("cpk_list.cfg.bin")` = `0x1717E18E`, donc **la clé est correcte**, pas le verrou) ; l'en-tête déchiffré est encore haute-entropie → chiffré+compressé autrement (tâche RE dédiée, ouverte). **Sans impact correctness** : l'accès aux assets passe par le TOC `@UTF` par-CPK (921/921 déchiffrent, 250 800 fichiers indexés ; `nie-model-serve` et `nie-game` le prouvent). `cpk_list.cfg.bin` ne serait qu'un index prêt-à-l'emploi (lookup sans scanner les 921 CPK) — gain de confort, pas une porte.
...
7. **RÉSOLU 2026-06-13** : l'overflow `cfgbin.rs:693` (panic/wrap sur en-tête chiffré) est corrigé (commit `7f3e09c`, +3 tests). `Vfs::init()` via `cpk_list.cfg.bin` reste KO (fichier chiffré+compressé non décodé) mais sans impact correctness (accès assets via TOC `@UTF` par-CPK). RE dédiée du conteneur `cpk_list.cfg.bin` = tâche ouverte.
