...
| `askama` | 0.14 | absent | templates compilés : `index.html` enrichi, erreurs, `robots.txt`, `sitemap.xml` |
...
- **Ce qui quitte le wiki** (vers nie) : `/cpk`, `/textures`, `/modeles`, `/mode`,
...
- **Redirections** : le wiki répond `308` vers `https://nie.aphrody.com<chemin>` pour les dix
...
## nie : `nie-site` sur `nie.aphrody.com`

**Ce que nie n'est pas** (précision de l'utilisateur, 2026-09-05) : ni le wiki — c'est
...
- **Rend** `/healthz` (JSON, ni nom ni version), `/robots.txt`, `/sitemap.xml`,
...
- **Sert `/downloads/inacord/latest.json`** : le manifeste de mise à jour d'Inacord, même
  ...
  nouvelles releases visent `nie.aphrody.com` en premier endpoint ; la route
...
- **Ne porte aucune marque Rose Griffon** : Inacord et nie sont des projets
...
|---|---|---|
| Coquille (shell) | le **menu principal** `mainmenu01` — capture ver. 7.1.2 (`data/design/aphrody-ui-ref-mainmenu-7.1.2.png`, hors dépôt) et sa décomposition dans `docs/DESIGN.md` | composants `SkewTile`, `TileRow`, `HeaderBanner`, `SidePanel`, `TitleBand`, `VersionChip`, `Callout`, `Badge` dans `packages/inacord-ui/src/shell/main-menu.tsx` (nie) ; `PhoneFrame`, `RoomList`, `MessageThread`, `HexBackdrop`, `TabBar` dans `shell/inacord.tsx` (Inacord) |
...
Aucun token, composant ou paquet `@rosegriffon/*` n'entre dans ce thème : nie et Inacord
...
`#A8CFD2` `#7B8F6B`. Deux coquilles (`shell/main-menu/` pour nie, `shell/inacord/` pour
...
1. un bloc `server_name nie.aphrody.com` → `proxy_pass http://127.0.0.1:8085`,
...
amender par son propriétaire — c'est bien ce sous-domaine que le site occupe.
