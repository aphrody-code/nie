...
## 2026-09-08 amendment — captured native startup and public truthfulness

The public root now follows the five chronological Windows reference captures transferred from
`data/menu`: loading, Inazuma Eleven logo, LEVEL5 logo, French autosave notice, and the complete
START screen. `apps/nie-web/scripts/prepare-opening-frames.ts` removes the measured one-pixel
window border and 32-pixel title bar to produce five lossless 1920×1080 assets with stable English
filenames. Four outputs have **0 differing pixels** against their source content crop. The LEVEL5
output differs on **9,010 / 2,073,600 pixels** because the Windows notification measured in its
otherwise uniform bottom-right background is removed. The originals remain untouched under
`data/menu` and are not committed.

The opening state machine has three timed frames followed by explicit autosave and START
confirmation. Only then does it mount the clean native reference
`data/menu/main_menu_alt.png`, copied byte-for-byte at build time and guarded by its
`b4500e…f4327507` SHA-256. The former handcrafted `MainMenu.tsx` and the provisional Wasm
...
2,286.8 KiB → 490.9 KiB** with Brotli.

Adjacent public truthfulness fixes remove implicit avatar parts/morphology, the custom mascot and
counts layered over `loading01`, three unsupported UI promises/counts, synthetic story dialogue,
and claims that the local `nie-runtime` simulation is the original match engine. Homepage metadata
now describes the Rust renderer as provisional; the public coverage endpoints
strip service/version fingerprints while preserving the internal on-disk matrix. Gates: `nie-app`
**23 passed / 0 failed** and `nie-wasm` **60 passed / 0 failed**, both with strict Clippy clean;
`nie-site` **347 passed / 0 failed / 1 ignored** with strict Clippy clean; the shared UI integrity
guard is **1 passed / 0 failed / 3 assertions**.

The public origin was traversed in Chromium at 1920×1080 on 2026-09-08: autosave and START both
require confirmation; the following surface reports `data-render-source="captured-reference"`,
contains no canvas, loads a 2560×1440 image whose live response has the same SHA-256, and routes
the two measured hit regions to `/avatar` and `/settings`. This proves exact capture delivery, not
that a static capture makes the still-incomplete C++/Lua renderer faithful to `nie.exe`.

The first crate-convergence pass also removed two real duplicates without collapsing distinct
responsibilities. The bounded PE inspector now lives in `aphrody-re`; `ievr-tools::pe::*` is a
compatibility re-export, so its Wasm caller remains source-compatible. The duplicated T2B-to-
iecode conversion and menu-layout ownership are being consolidated into their format library
instead of their binary consumers. No whole workspace crate is currently deletion-safe: Cargo
metadata measures **41 members (19 engine + 10 forge + 12 tools)** and each apparent candidate is
either a library consumer, an executable binding, or a compatibility surface still covered by a
gate.

The same evidence rule was applied outside the menu path. `nie-index` now owns the configurable
RE database fallback consumed by the CLI/MCP binding. The duplicated episode-navigation rules in
Wonderbot and Inacord now live in one browser-safe module while both historical APIs remain
compatible; Wonderbot reports **223 passed / 0 failed / 499 assertions**, and both package
typechecks are clean. A Git-history audit found that Inacord already retains the useful earlier
work: lazy views, cancellable persistent VFS indexing, the shared 3D viewport and G4MD/G4MG
...
et `/api/v1/modes` consomment directement les mêmes slugs, préfixes VFS, hashes, libellés, notes
et indicateurs officiels. L'adaptateur CLI qui dupliquait ces champs a été supprimé. Les gates
rejouées le 2026-09-08 donnent **2/2** tests de catalogue et **2/2** tests CLI de compatibilité,
avec Clippy strict vert. La prochaine extraction utile reste le compilateur de layout
asset-agnostique de `nie-game`, pas les bindings hôte.
...
composant React qui mélangeait textures VFS et valeurs joueur inventées a été supprimé : l'export
reste l'oracle de convergence du futur renderer de menu, mais n'est pas la surface de démarrage
visible. Les deux commandes menu inconnues du
...
> **Documentation VFS réalignée — 2026-09-07.** `var/couverture-site.json` mesure désormais
> 249 787 fichiers servis, 5 512 internes et 9 bloqués sur 255 308 ; `docs/VFS.md` a été
