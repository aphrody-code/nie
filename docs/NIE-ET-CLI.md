# Manuel de Référence : nie.exe et la CLI `niers`

Ce document présente l'architecture de **`nie.exe`** (le binaire cible original d'Inazuma Eleven: Victory Road) et le guide d'utilisation opérationnel complet de la CLI **`niers`** (le binaire Rust unique unifié).
...
## 2. La CLI `niers` : Principe et Fonctionnement

La CLI `niers` (fournie par le crate `crates/tools/nie-cli`) est **le point d'entrée unique** de l'outillage Rust. Elle unifie l'exploration du jeu réel, l'analyse statique et dynamique, le décodage de formats et la boucle de reverse-engineering.
...
```
Sur une installation Windows Steam par défaut, `niers` détecte automatiquement le VFS dans le répertoire courant si `data/cpk_list.cfg.bin` est présent.

---

## 3. Guide des Commandes de la CLI `niers`

### 3.1 Exploration et Extraction du VFS (`niers vfs`)
...
- **Inspection et décodage structuré à la volée (`cat`)** :
  ...
  niers vfs cat data/common/text/fr/menu_text.cfg.bin

  # Inspection des métadonnées d'un modèle 3D (G4MD)
  niers vfs cat data/common/chr/_animal/an000100/an000100.g4md

  # Décodage direct d'une texture G4TX vers un PNG
  niers vfs cat data/dx11/menu/200_icon/10_icon_chr/face/c01001900_l.g4tx --png-out /tmp/aphrody.png
  ```

- **Recherche transversale d'un Personnage (`chara`)** :
  ```bash
  niers vfs chara Byron
  ```
...
- **Détection de format (`format`)** :
  ```bash
  niers format data/common/text/fr/menu_text.cfg.bin
  ```
  Identifie le type exact d'un fichier (magic, structure interne, codec).

- **Décodage en masse vers JSON / PNG (`decode`)** :
  ```bash
  niers decode chemin/fichier.g4tx -o destination/
  niers decode chemin/fichier.cfg.bin
  ```
...
- **Conversion d'assets (`convert`)** :
  ```bash
  niers convert model.g4md -o model.glb
  ```
...
### 3.5 Pilotage du Savoir Reverse-Engineering (`niers atlas`)
...
- **Interrogation unifiée de l'Atlas (`search`)** :
  ```bash
  niers atlas search "CCameraCtrl"
  niers atlas search "0x1404ecd60"
  ```
