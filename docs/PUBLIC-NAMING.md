...
`niers`; neither is a public executable name.
...
| Repository root | resolved by `NIE_REPO_ROOT` | legacy `NIERS_VPS_REPO` accepted by sync only |
...
mention `niers`, but new public commands, manifests and release archives use `nie`.
