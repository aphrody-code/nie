...
- `~/.config/niers/environment.env` contains non-secret machine paths and is sourced by Bash.
...
`~/.config/niers/`, installs symlinks for built Rust commands into `~/.local/bin`, enables the
...
The repository-scoped `.envrc`, Codex config, and Claude config activate niers only when the
...
serializes launches, enables niers only for a working directory below this checkout, and disables
...
direnv exec . bash -lc 'command -v niers nie-mcp steam steamcmd cargo bun uv codex claude agy'
```

Rollback consists of removing the marked block from `~/.bashrc`, removing
`~/.config/niers/environment.env`, running `direnv deny`, and removing the local `.env` symlink.
Remove `/etc/profile.d/niers-path.sh` with administrator privileges to roll back the system PATH,
