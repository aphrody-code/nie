...
`text/` holds the OC dialogue event `ev98_99010` in the game's own `data/common/text` shape —
fifteen lines transcribed from the comic pages, in French and English, plus the washa map that
joins them. It is generated by `scripts/donnees/astro-lor-dialogue.py`; the format, the CRC-32
of the washa label used as the row hash, and what remains unproven are all recorded in
