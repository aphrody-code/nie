    ...
    pub fn try_build(&self, version: Version) -> Result<Artifacts, Box<dyn Error>> {
        ...
        match target {
            ...
            _ if target.ends_with("emscripten") => {
                // PATCHED FOR THIS REPOSITORY — see crates/engine/nie-lua-web/README.md.
                //
                // Upstream compiles Lua as C++ here (`.cpp(true).flag("-fexceptions")`, plus a
                // copy of every source into `cpp_source/` with the public headers wrapped in
                // `extern "C"`), so that `LUAI_THROW` becomes a C++ `throw`.
                //
                // In a browser that is fatal, and measured: the throw unwinds through the
                // C++/wasm unwinder, `mlua`'s `catch_unwind` meets it as a FOREIGN exception,
                // and Rust aborts by design — `fatal runtime error: Rust cannot catch foreign
                // exceptions`. Seven rebuilds, a nightly bumped from 1.98 to 1.100 and every
                // exception-mode flag combination left that untouched, because the cause is
                // here and not in the flags.
                //
                // Compiled as plain C, `LUAI_THROW` is `setjmp`/`longjmp` again, which
                // emscripten implements without C++ unwinding. Nothing else changes: the same
                // sources, the same `LUA_USE_POSIX`.
                // Le mode de `longjmp` appartient à la COMPILATION, pas à l'environnement :
                // `CFLAGS_wasm32_unknown_emscripten` n'entre pas dans l'empreinte de cargo, donc
                // le poser hors d'ici ne recompile rien (mesuré : même `ldo.o`, même empreinte).
                // Il doit correspondre au lien, qui suit `-fwasm-exceptions` de rustc.
                config
