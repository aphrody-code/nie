...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error(`[niers-game] AVERTISSEMENT index VFS indisponible : ${(e as Error).message}`);
  }
  ...
  const requireVfs = (): VfsIndex => {
    if (!vfs) throw new ToolError("index VFS indisponible (Redis injoignable au démarrage)");
    return vfs;
  };
