...
function LazyImage({
	...
	// Handle "inherit" case for next/image
	const finalWidth = width === "inherit" ? maxWidth : width;
	const finalHeight = height === "inherit" ? 400 : height; // Fallback height
	...
	return (
		<NextImage
			...
			onError={onError}
			draggable="false"
			unoptimized // External URLs or dynamic ones often need this if not configured in remotePatterns
		/>
	);
}
				...
				<div className="image-caption-container absolute right-0 bottom-1 left-0 m-0 block min-w-[100px] overflow-hidden border-t bg-white/90 p-0">
					...
					>
						...
						<RichTextPlugin
							contentEditable={
								<ContentEditable
									className="ImageNode__contentEditable user-select-text word-break-break-word caret-primary relative block min-h-5 w-[calc(100%-20px)] cursor-text resize-none border-0 p-2.5 text-sm whitespace-pre-wrap outline-hidden"
