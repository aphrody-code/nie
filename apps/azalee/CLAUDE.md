...
- **Déploiement VPS (PAS Vercel)** : Azalee tourne en self-host sur le VPS via `azalee-web.service` (Next 16 standalone, port 3003). Deploy = `bash scripts/ship-azalee.sh` (build → `deploy-azalee.sh` standalone → `systemctl restart azalee-web.service` → healthcheck `127.0.0.1:3003`). **Ne JAMAIS déployer azalee sur Vercel** (le projet `rg-azalee` Vercel est abandonné, ses builds échouent). Le domaine `azalee.rosegriffon.fr` pointe sur le VPS via nginx.
