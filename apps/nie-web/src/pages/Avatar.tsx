...
/* ── Composant Diagramme Hexagonal Officiel Level-5 ───────────────────────── */
function Level5HexagonalRadar({
	stats,
	baseStats,
	combatPower,
	rank,
	...
	const center = size / 2;
	const maxR = 90;
	const maxStatValue = 260;

	const keys: (keyof PlayerStats)[] = ["frappe", "puissance", "precision", "vitesse", "defense", "gardien"];
	const labels = ["FRAPPE", "PUISSANCE", "PRÉCISION", "VITESSE", "DÉFENSE", "GARDIEN"];

	const getCoordinates = (value: number, index: number) => {
		const angle = -Math.PI / 2 + (index * Math.PI) / 3;
		const r = Math.min(maxR, Math.max(15, (value / maxStatValue) * maxR));
		return {
	...
	const basePoints = keys.map((k, i) => {
		const pt = getCoordinates(baseStats[k], i);
		return `${pt.x.toFixed(1)},${pt.y.toFixed(1)}`;
	}).join(" ");

	const boostedPoints = keys.map((k, i) => {
		const pt = getCoordinates(stats[k], i);
		return `${pt.x.toFixed(1)},${pt.y.toFixed(1)}`;
	}).join(" ");
	...
	return (
		<div className="level5-radar-container">
			<div className="level5-radar-header">
				<div className="level5-radar-title">
					<span>⚡ STATISTIQUES LEVEL-5</span>
				</div>
				<div className="level5-power-badge">
					<span className="level5-rank-pill">RANG {rank}</span>
					<span>{combatPower.toLocaleString()} CP</span>
				</div>
			</div>

			<div className="level5-radar-svg-wrap">
				<svg className="level5-radar-svg" viewBox={`0 0 ${size} ${size}`}>
					{gridRings.map((pts, idx) => (
						<polygon
							key={idx}
							points={pts}
							fill="transparent"
							stroke="rgba(1, 254, 204, 0.16)"
					...
					{keys.map((_, i) => {
						...
						return (
							<line
								...
								x2={ex}
								y2={ey}
								stroke="rgba(1, 254, 204, 0.25)"
								strokeWidth="1"
							/>
						);
					})}

					<polygon points={basePoints} className="radar-poly-base" />

					{boostedPoints !== basePoints && (
						<polygon points={boostedPoints} className="radar-poly-boosted" />
					)}

					{keys.map((k, i) => {
						const pt = getCoordinates(stats[k], i);
						const basePt = getCoordinates(baseStats[k], i);
						const isBoosted = stats[k] > baseStats[k];
						return (
							<g key={k}>
								<circle
									cx={basePt.x}
					...
					{keys.map((k, i) => {
						...
						return (
							<g key={`lbl-${k}`}>
								<text x={lx} y={ly - 6} className="radar-stat-label">
									{labels[i]}
								</text>
								<text
									x={lx}
									y={ly + 6}
									className={`radar-stat-value ${isBoosted ? "radar-stat-value-boosted" : ""}`}
								>
									{stats[k]}
									{isBoosted ? ` (+${stats[k] - baseStats[k]})` : ""}
								</text>
							</g>
						);
					})}
			...
			<div className="level5-stat-breakdown">
				{keys.map((k, i) => {
					const bonus = stats[k] - baseStats[k];
					return (
						<div key={k} className={`level5-stat-chip ${bonus > 0 ? "boosted" : ""}`}>
							<div className="level5-stat-chip-label">
								<span>{labels[i]}</span>
								{bonus > 0 && <span className="level5-stat-chip-bonus">+{bonus}</span>}
							</div>
							<div className="level5-stat-chip-num">
								<span>{stats[k]}</span>
							</div>
						</div>
					);
...
/* ── Canvas Interactif d'Aura & Particules Luminescentes ─────────────────── */
function AuraCanvasOverlay({
	active,
	keshinActive,
	auraColor,
	mixiMaxColor,
	...
	useEffect(() => {
		...
		const maxParticles = keshinActive ? 65 : 40;
		const colors = [auraColor];
		if (mixiMaxColor) colors.push(mixiMaxColor);
		colors.push("#ffffff", "#ffd700");
		...
		const spawnParticle = (): Particle => {
			...
			return {
				...
				y: cy + (Math.random() - 0.5) * (height * 0.2),
				...
				life: 0,
				maxLife: Math.random() * 60 + 35,
				color: colors[Math.floor(Math.random() * colors.length)] ?? "#01fecc",
			};
		};
		...
		const render = () => {
			...
			const ringRadius = 90 + Math.sin(pulseAngle) * 18;
			ctx.save();
			...
			if (mixiMaxColor) {
				const ring2 = 120 + Math.cos(pulseAngle * 1.3) * 22;
				ctx.beginPath();
				ctx.arc(0, 0, ring2, 0, Math.PI * 2);
				ctx.strokeStyle = mixiMaxColor;
				ctx.lineWidth = 2.5;
				ctx.shadowColor = mixiMaxColor;
				ctx.shadowBlur = 24;
				ctx.stroke();
			}
			ctx.restore();
	...
	}, [active, keshinActive, auraColor, mixiMaxColor]);

	if (!active) return null;

	return <canvas ref={canvasRef} className="aura-viewport-canvas-overlay" />;
}
