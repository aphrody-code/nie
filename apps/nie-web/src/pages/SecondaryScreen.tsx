		...
		>
			<header
				style={{
					...
					background: "linear-gradient(180deg, var(--jeu-surface-glace), var(--jeu-ciel-brume))",
			...
			>
				...
				>
					...
					{entries.map((entry) => (
						<IconTile
							key={entry.route}
							icone={GLYPHES[entry.glyph]}
							libelle={entry.label}
							actif={entry.route === currentView}
								largeur={TILE_WIDTH}
								hauteur={TILE_HEIGHT}
								penche={bevel(TILE_HEIGHT)}
