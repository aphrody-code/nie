...
function decoderGlb(buffer: ArrayBuffer): ModeleGl {
	...
	while (o + 8 <= buffer.byteLength) {
		...
		if (type === 0x4e4f534a)
			json = JSON.parse(new TextDecoder().decode(new Uint8Array(buffer, debut, taille)));
	...
	function lire(
	...
	): { data: Float32Array; count: number; comp: number } | null {
		...
		const echelle = a.normalized
			? { 5120: 127, 5121: 255, 5122: 32767, 5123: 65535 }[
					a.componentType as 5120 | 5121 | 5122 | 5123
				]
			: undefined;
	...
	for (const img of racine.images ?? []) {
		...
		images.push(
			new Blob([new Uint8Array(buffer, debut, v.byteLength ?? 0)], {
...
function argile(device: GPUDevice): GPUTexture {
	...
	device.queue.writeTexture(
		{ texture: t },
		new Uint8Array([206, 198, 188, 255]),
		{ bytesPerRow: 4 },
		[1, 1]
	);
	return t;
...
function Viewport({
	...
	useEffect(() => {
		...
		if (!navigator.gpu) {
			setEtat("echec");
			setRaison(
				"Ce navigateur n'expose pas WebGPU. L'aperçu rendu par le serveur reste disponible."
			);
			return;
		}
		...
		(async () => {
			...
			if (longueur > GLB_OCTETS_MAX)
				throw new Error(`modèle trop lourd pour le navigateur (${octets(longueur)})`);
			const buffer = await r.arrayBuffer();
			if (!vivant) return;
			const decode = decoderGlb(buffer);
			if (decode.primitives.length === 0)
				throw new Error("aucune géométrie exploitable dans ce modèle");
			...
			const bitmaps = await Promise.all(
				decode.images.map((b) =>
					b.size > 0 ? createImageBitmap(b).catch(() => null) : Promise.resolve(null)
				)
			);
			if (!vivant) return;
			...
			for (const bmp of bitmaps) {
				...
				dev.queue.copyExternalImageToTexture({ source: bmp }, { texture: t }, [
					bmp.width,
					bmp.height,
				]);
				bmp.close();
	...
	const onWheel = useCallback((e: React.WheelEvent<HTMLCanvasElement>) => {
		vue.current.distance = Math.min(
			9,
			Math.max(1.4, vue.current.distance + Math.sign(e.deltaY) * 0.2)
		);
	}, []);
