...
function familles(facettes: Facette[]): GameFilterFamily[] {
	...
	return [
		{ id: "ext", label: "Extension", icon: GLYPHES.livre, options: valeurs("ext") },
		{
			...
			label: "Pack",
			icon: GLYPHES.cube,
			options: valeurs("cpk").map((o) => ({
				...o,
				label: o.value.replace(/\.cpk$/, "").slice(0, 12),
			})),
		},
		{
			id: "tri",
...
export function Explorateur() {
	...
	const { prefixe, q, ext, tri, ordre, minMo, maxMo, glob, cpk, partout, choisi, vue, cote } = etat;
	...
	if (!capacites.vfs) {
		return (
			<Notice>
				L'arborescence est en cours de préparation. Elle s'affichera dès qu'elle sera prête.
			</Notice>
		);
	}
	...
	return (
		<section>
			<ViewTitle detail={contenu || globaux ? agree(retenus, "fichier") : undefined}>
				Explorer
			</ViewTitle>
			...
			<nav aria-label="Chemin" style={{ margin: "0 0 var(--jeu-espace-s)", fontSize: "0.9rem" }}>
				...
				{segments.map((s) => (
					<span key={s.chemin}>
						...
						<button
							type="button"
							onClick={() => modifier({ prefixe: s.chemin, choisi: "" })}
							style={LIEN}
						>
							{s.nom}
						</button>
			...
			<p style={{ margin: "0 0 var(--jeu-espace-m)", fontSize: "0.9rem", opacity: 0.8 }}>
				{globaux
					? `${agree(retenus, "fichier")} dans tout le sous-arbre`
					: filtreActif
						? `${agree(retenus, "fichier")} sur ${avantFiltre}`
						: agree(retenus, "fichier")}
				{extIntrouvable ? " — aucun fichier de ce type dans ce dossier" : ""}
			</p>

			{erreur ? (
				<Notice tone="alerte">Ce dossier n'a pas pu être ouvert. Réessayez dans un instant.</Notice>
			) : null}
			...
			>
						...
						<div tabIndex={0} onKeyDown={auClavier} style={{ outline: "none" }}>
							...
							{fichiers.length > visibles ? (
								...
								<p style={{ fontSize: "0.85rem", opacity: 0.75 }}>
									{fichiers.length.toLocaleString("fr")} des {retenus.toLocaleString("fr")} affichés
									— affinez le filtre pour voir les suivants.
								</p>
							) : null}
						</div>
					)}
				...
				>
					...
					<details style={{ marginTop: "var(--jeu-espace-m)" }}>
						...
						<DataPanel context={choisi ? codeDe(choisi) : undefined} />
					</details>
				</aside>
			</div>
		</section>
	...
	return (
		<>
			...
			<dl style={{ margin: 0 }}>
				...
				<Ligne cle="Poids affiché" valeur={readableSize(poids)} />
			</dl>
...
function PanneauAsset({ chemin, fichier }: { chemin: string; fichier: EntreeVfs | null }) {
	...
	return (
		<>
			<h2 style={{ ...TITRE_PANNEAU, wordBreak: "break-all" }}>{chemin.split("/").pop()}</h2>
			{estTexture ? (
				<img
			...
			<dl style={{ margin: 0 }}>
				...
				{fichier ? <Ligne cle="Taille" valeur={readableSize(fichier.taille)} /> : null}
				{fichier?.cpk ? <Ligne cle="Pack" valeur={fichier.cpk} coupe /> : null}
				<Ligne cle="Format lu" valeur={decodage?.format ?? (refus ? "—" : "…")} />
				{decodage?.octets ? (
					<Ligne cle="Octets lus" valeur={decodage.octets.toLocaleString("fr")} />
				) : null}
			</dl>
...
function Ligne({ cle, valeur, coupe }: { cle: string; valeur: string; coupe?: boolean }) {
	return (
		<div
			style={{ display: "flex", gap: "var(--jeu-espace-s)", fontSize: "0.85rem", padding: "2px 0" }}
		>
