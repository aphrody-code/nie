 ...
 *
 * Le fichier exporté est embarqué tel quel (`../layouts/loading01.layout.json`), comme
...
import { CanvasItem, GameCanvas, LayoutRender, lireLayout as readLayout } from "@niers/inacord-ui";
import { useMemo } from "react";
import rawLayout from "../layouts/loading01.layout.json";
import { type Humeur as Mood, PetAphrody } from "./PetAphrody";
 ...
 */
const BAND_TOP = LAYOUT.canvas.h / 2 - 136 / 2;
const PET_Y = BAND_TOP - 32;

/** La ligne de texte, sous la bande. */
const LINE_Y = LAYOUT.canvas.h / 2 + 136 / 2 + 40;

/** Ce que l'écran a le droit de dire, pour chaque état que le serveur publie réellement. */
type VfsState = SiteHealth["capacites"]["vfs"];
 ...
 */
const MESSAGE_BY_STATE: Record<VfsState, string> = {
...
function moodFor(state: VfsState | null, failed: boolean): Mood {
	if (failed || state === "absent") return "panne";
	if (state === "pret") return "repos";
 ...
 */
export function Loading({ health, failed = false, title }: LoadingProps) {
	const vfs = health?.capacites.vfs ?? null;
	const mood = moodFor(vfs, failed);

	const line = useMemo(() => {
		if (failed) return "Le site ne parvient pas à joindre ses ressources.";
		if (!vfs) return "Mesure en cours.";
		return MESSAGE_BY_STATE[vfs];
	}, [failed, vfs]);
	...
	const entryCount = health?.capacites.vfs_entrees ?? 0;
	...
	return (
		<GameCanvas canvas={LAYOUT.canvas} fond="var(--jeu-ciel-clair)">
			...
			<CanvasItem x={LAYOUT.canvas.w / 2} y={PET_Y} ancreX={0.5} ancreY={1} z={400}>
				<PetAphrody humeur={mood} />
			</CanvasItem>

			<CanvasItem x={LAYOUT.canvas.w / 2} y={LINE_Y} ancreX={0.5} ancreY={0} z={400}>
				...
				>
					...
					{entryCount > 0 ? (
						<span style={{ fontSize: "15px", opacity: 0.8 }}>
							{entryCount.toLocaleString("fr-FR")} entrées indexées
						</span>
					) : null}
				</div>
			</CanvasItem>
