...
describe("styleObjet", () => {
	...
	test("l'ordre des fonctions place l'ancre sur (x, y) MALGRE l'echelle", () => {
		...
		const s = styleObjet(
			objet({
				transform: { x: 10, y: 20, anchorX: 0.5, anchorY: 0.5, scaleX: 0.66, scaleY: 0.66, rot: 0 },
			})
		);
...
describe("dansCanvas / auCentreParDefaut / estMuet", () => {
	...
	test("le point d'ancrage decide", () => {
		expect(
			dansCanvas(
				objet({
					transform: { x: 640, y: 360, anchorX: 0.5, anchorY: 0.5, scaleX: 1, scaleY: 1, rot: 0 },
				}),
				canvas
			)
		).toBe(true);
		expect(
			dansCanvas(
				objet({
					transform: { x: 2975, y: 1742, anchorX: 0.5, anchorY: 0.5, scaleX: 1, scaleY: 1, rot: 0 },
				}),
				canvas
			)
		).toBe(false);
	...
	test("le centre exact signale une position jamais posee par l'export", () => {
		...
		expect(
			auCentreParDefaut(
				objet({
					transform: { x: 641, y: 360, anchorX: 0.5, anchorY: 0.5, scaleX: 1, scaleY: 1, rot: 0 },
				}),
				canvas
			)
		).toBe(false);
...
describe("bilanLayout sur mainmenu01", () => {
	...
	test("le runtime ne garde aucun objet visible muet", () => {
		// Les valeurs directes de `SetText` et `SetObjectNum` sont normalisées avant le rendu.
		expect(bilan.muets).toBe(0);
	});

	test("le runtime reduit les positions par defaut a 7 et les sorties de canevas a 2", () => {
		expect(bilan.auCentre).toBe(7);
		expect(bilan.horsCanvas).toBe(2);
	});

	test("13 textures distinctes couvrent le menu principal et ses composants partages", () => {
		expect(bilan.textures.length).toBe(13);
		expect(bilan.textures.every((t) => t.startsWith("data/dx11/menu/"))).toBe(true);
	});
});
