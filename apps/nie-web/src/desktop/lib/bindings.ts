...
export const commands = {
	sqliteLoad: (db: string) => typedError<string, string>(__TAURI_INVOKE("sqlite_load", { db })),
	sqliteSelect: (db: string, query: string, values: JsonValue[]) => typedError<JsonValue[], string>(__TAURI_INVOKE("sqlite_select", { db, query, values: values.map(i=>i) })).then((v) => ((v.status === "ok" ? { ...v, data: v.data.map(i=>i) } : v) as typeof v)),
	sqliteExecute: (db: string, query: string, values: JsonValue[]) => typedError<JsonValue, string>(__TAURI_INVOKE("sqlite_execute", { db, query, values: values.map(i=>i) })),
	sqliteClose: (db: string | null) => typedError<boolean, string>(__TAURI_INVOKE("sqlite_close", { db })),
