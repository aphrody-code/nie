  ...
  test("renders the canonical page hierarchy and list surface", () => {
    const container = render(
      ...
      >
        ...
        >
		  <ExplorerEntries viewMode="list" activeDescendant="file-a"><button id="file-a" role="option" aria-selected="true" tabIndex={-1} type="button">action</button></ExplorerEntries>
        </ExplorerSurface>
      </ExplorerPageShell>,
    );
	...
	const entries = container.querySelector('[role="listbox"][aria-label="Files and folders"]');
	expect(entries?.getAttribute("aria-activedescendant")).toBe("file-a");
	expect(entries?.querySelectorAll('[role="option"]')).toHaveLength(1);
