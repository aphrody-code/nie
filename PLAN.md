...
| Editor viewport | **`nie-render3d`** through `RustSceneViewport`, three.js kept as fallback | grid, wireframe, selection outline and a translate/rotate/scale gizmo, drawn by a `LineList` pipeline. `createSceneViewer` stops the fallback chain at WebGL 2 on purpose — the CPU tier has no `load_scene`, `pick_json` or gizmo, so it would build a viewport that fails on the first click. Still missing on the Rust path: `referenceImage`, and per-node triangle counts (reported as zero) |
