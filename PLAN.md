...
| Wiki presentation cards already represented in the package: tactics, passives, quests, stadiums, stat curve/heptagon, media shell, model toolbar, gallery card, video actions/player and filters | `packages/inacord-ui/src/components/wiki/wiki` | **Shared — partially migrated** | Use one visual component with injected data and navigation adapters. Complete the remaining card/detail callers before deleting an Azalee copy. |
| Wiki cards still Azalee-only: `Character*`, `Coach*`, `Item*`, `Skill*`, `Team*`, `MovesetList`, `OverrideSkillSection`, `Shop*`, `GachaCard`, `DropsCard`, `FormSelector`, `GaijiGlyph` | `packages/inacord-ui/src/components/wiki/wiki` | **Migrate** | Extract display props from Supabase queries; keep SQLite/Supabase/VFS joins in the relevant host/data library. Every entity view must show resolved native name plus immutable ID. `AuraCard` and `ElementIcon` are already shared; their CDN and VFS image pipelines remain thin host adapters. |
| Wiki list/search controls: `GlobalSearch`, `CharacterSearchDialog`, `WikiSearchToolbar`, `SearchResult*`, `CharacterTable`, `BaseCharacterTable`, `TeamsListClient`, `AuraList` | `packages/inacord-ui/src/components/wiki/wiki` plus `components/ui/{command,data-grid,tree-rows}` | **Migrate** | Share query-independent rows, filter state and result rendering. Azalee retains URL/SSR search parameters; Inacord supplies its desktop/router and local-index adapters. |
| Wiki filter families: `CharacterFilters`, `ItemFilterBar`, `PassiveFilters`, `PassivePlayerFilters`, `RarityFilterChips`, `SkillFilterBar`, `TeamFilter`, `QuestFilterBar` | `packages/inacord-ui/src/components/wiki/wiki/filters` | **Migrate / merge** | Retain the existing shared item/passive/skill/quest filters, add missing character/rarity/team filters there, and inject facet queries from the host. |
...
| Azalee APIs and server actions: Supabase/GraphQL/RAG, articles, comments, newsletter, notifications, social/share tracking, cron, image upload, VRoid and administration | Rust data libraries where portable; otherwise Azalee server adapter | **Host-only until a portable domain owner exists** | No Next route or Server Action is copied to Tauri. First extract a tested library for a portable rule; retain HTTP/auth/storage/cron code in Azalee. |
