...
The standing goal names three pillars: *full wasm*, *Rust owning Inacord and Azalée*, and *real
...
| | count |
| --- | --- |
| screens in `data/menu/manifest.json` `entries` | 33 |
| families the Lua runtime can replay | 14 |
| families identical browser vs native (`differential.ts`) | 10 |
| screens the browser actually paints | **3** |

The gap between 14 and 3 is not compositor work — layout, replay and painting already agree
cross-host for those families. It is that each screen binds its own game data (shops and items,
characters, trophies), so the three existing screens are not three copies of one component and
cannot be collapsed into a generic one. Each additional family costs its own data binding.

Even on the three that exist, the split runs *inside* the screen: the Rust compositor paints the
chrome (background, plates, frames, text labels) and the React host paints the content (list
...
| Game assets in a Bevy app | **`nie-bevy`** (`--features bevy`), an AssetReader over the game VFS | `nie://…/c11010010.g4md` loads straight out of the CPK archives into a headless Bevy App: 3 submeshes, vertices [1306, 440, 76], 445µs. A real atlas becomes a Bevy `Image` 1728×352. Off by default — `cargo check -p nie-bevy` compiles zero Bevy crates, and `cargo tree -i bevy_ecs` is empty for nie-runtime, nie-core, nie-net and nie-formats. **No consumer yet** — it is a proven bridge, not a wired one |
| Editor viewport | **`nie-render3d`** through `RustSceneViewport`, three.js kept as fallback | grid, wireframe, selection outline and a translate/rotate/scale gizmo, drawn by a `LineList` pipeline. `createSceneViewer` stops the fallback chain at WebGL 2 on purpose — the CPU tier has no `load_scene`, `pick_json` or gizmo, so it would build a viewport that fails on the first click. Feature parity with the three.js viewport reached 2026-09-20: per-node triangle counts come from `scene_stats_json`, and `referenceImage` is a layer under a transparent canvas — the transparency already existed for avatars, `createSceneViewer` was passing `false` |
